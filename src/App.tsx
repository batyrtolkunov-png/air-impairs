      ...
      {menuOpen && menuTab === "home" && (
        <div className="menu-tab-overlay">
          <section className="menu-tab-panel home-tab-panel">
            ...
            <h2>ПИТОМЦЫ</h2>
            <div className="locked-pets-grid">{Array.from({length:5},(_,index)=>{const catUnlocked=index===0&&completedRegions>=1,crocUnlocked=index===1&&completedRegions>=2,pugUnlocked=index===2&&completedRegions>=3;return <article className={`locked-cat cat-${index+1} ${catUnlocked?'pet-unlocked black-cat-pet':''} ${crocUnlocked?'pet-unlocked crocodile-pet':''} ${pugUnlocked?'pet-unlocked watermelon-pug-pet':''}`} key={index}>{pugUnlocked?<div className="pug-shadow"><i className="pug-tail"/><i className="pug-body"/><i className="pug-leg left"/><i className="pug-leg right"/><i className="pug-head"/><i className="pug-ear left"/><i className="pug-ear right"/><i className="pug-muzzle"/><i className="pug-eye left"/><i className="pug-eye right"/><i className="watermelon-hat"/><i className="watermelon-stripe one"/><i className="watermelon-stripe two"/></div>:crocUnlocked?<div className="crocodile-shadow"><i className="croc-tail"/><i className="croc-body"/><i className="croc-head"/><i className="croc-jaw"/><i className="croc-eye left"/><i className="croc-eye right"/><i className="croc-leg leg-a"/><i className="croc-leg leg-b"/><i className="croc-leg leg-c"/><i className="croc-leg leg-d"/></div>:<div className="cat-shadow"><i className="cat-ear left"/><i className="cat-ear right"/><i className="cat-head"/><i className="cat-eye left"/><i className="cat-eye right"/>{catUnlocked&&<i className="cat-mouth"/>}<i className="cat-body"/><i className="cat-paw left"/><i className="cat-paw right"/><i className="cat-tail"/></div>}<strong>{catUnlocked?'ЧЁРНАЯ КОШКА':crocUnlocked?'КРОКОДИЛ':pugUnlocked?'АРБУЗНЫЙ МОПС':'НЕИЗВЕСТНЫЙ ПИТОМЕЦ'}</strong><span>{catUnlocked?'СКОРОСТЬ 1,5× · УКУС':crocUnlocked?'СКОРОСТЬ 1,65× · УКУС':pugUnlocked?'АТАКА ГОЛОВОЙ':'???'}</span></article>})}</div>
            <p>{completedRegions>=3?'Открыт Арбузный мопс. Он разгоняется и бьёт врагов головой.':completedRegions>=2?'Чёрная кошка и крокодил открыты. Третий питомец ждёт в Ледяном кладбище.':completedRegions>=1?'Чёрная кошка открыта за прохождение Тёмного леса. Следующий питомец скрыт в пустыне.':'Проходи локации, чтобы тени превращались в настоящих питомцев.'}</p>
          </section>
        </div>
      )}

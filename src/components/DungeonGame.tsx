...
function enemyHitRadius(enemy: Enemy) { return enemy.kind === 'boss' ? BOSS_HITBOX_RADIUS : enemy.kind === 'goblin' || enemy.kind === 'monkey' || enemy.kind === 'nativeSpear' || enemy.kind === 'nativeClub' || enemy.kind === 'knightGuard' || enemy.kind === 'mummy' || enemy.kind === 'iceGolem' || enemy.kind === 'mudMonster' ? 18 : 14; }
...
function drawKnightGuard(ctx: CanvasRenderingContext2D,e:Enemy,now:number,attacking:boolean){const stride=Math.sin(now/120+e.x*.03)*4;ctx.fillStyle='rgba(0,0,0,.32)';ctx.beginPath();ctx.ellipse(64,111,39,9,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#252c35';ctx.fillRect(40,22,49,43);ctx.fillStyle=e.flash>0?'#fff':'#aebdca';ctx.fillRect(44,19,41,39);ctx.fillRect(38,59,53,43);ctx.fillStyle='#dce7ec';ctx.fillRect(48,23,33,7);ctx.fillStyle='#202833';ctx.fillRect(48,36,33,8);ctx.fillStyle='#f2c15d';ctx.fillRect(54,38,6,4);ctx.fillRect(70,38,6,4);ctx.fillStyle='#8c263b';ctx.fillRect(59,7,12,18);ctx.fillRect(52,10,26,8);ctx.fillStyle='#6d7884';ctx.fillRect(27,62,13,37);ctx.fillRect(90,62,13,37);ctx.fillStyle='#843245';ctx.fillRect(43,70,43,27);ctx.fillStyle='#d4b354';ctx.fillRect(43,68,43,6);ctx.fillStyle='#343b46';ctx.fillRect(43,97+stride,18,14);ctx.fillRect(70,97-stride,18,14);const swing=attacking?-.9:-.18;ctx.save();ctx.translate(101,66);ctx.rotate(swing);ctx.fillStyle='#634126';ctx.fillRect(-5,-4,10,61);ctx.fillStyle='#becbd0';ctx.beginPath();ctx.moveTo(-5,-18);ctx.lineTo(-28,-8);ctx.lineTo(-22,13);ctx.lineTo(5,8);ctx.closePath();ctx.fill();ctx.fillStyle='#f3fbff';ctx.fillRect(-21,-6,14,4);ctx.restore();}
...
function drawScene(ctx: CanvasRenderingContext2D, map: ReturnType<typeof getLevel>, hero: Point, enemies: Enemy[], projectiles: Projectile[], superFists: SuperFist[], swordUltimates: SwordUltimate[], bowUltimates: BowUltimate[], staffUltimates: StaffUltimate[], glovesUltimates: GlovesUltimate[], waves: MagicWave[], sandTornadoes: SandTornado[], tombs: Tomb[], now: number, openedChests: number[], chestDrops: LootDrop[], attackProgress: number, loot: Point | null, droppedItem: Weapon | null, level: number, weapon: Weapon | null, weapon2: Weapon | null, armor: Weapon | null, facing: Point, moving: boolean, health: number, superReloading: boolean, profileName: string, secondHero: Point | null, secondFacing: Point, secondMoving: boolean, secondHealth: number, superReloading2: boolean, attackProgress2: number, armor2: Weapon | null, skin: HeroSkin, skin2: HeroSkin, explored: Point[], mobileControls = false, cameraFocus: Point = hero, castleGuardVisible = true) {
  ...
  enemies.forEach((e) => {
    ...
    else { ctx.translate(e.x + 1 + (hero.x - e.x) / distance * force, e.y - 5 + (hero.y - e.y) / distance * force); const enemyScale = e.kind === 'goblin' || e.kind === 'monkey' || e.kind === 'nativeSpear' || e.kind === 'nativeClub' || e.kind === 'knightGuard' || e.kind === 'mummy' || e.kind === 'iceGolem' || e.kind === 'iceSpirit' || e.kind === 'frog' || e.kind === 'mudPile' || e.kind === 'mudMonster' ? .38 : e.kind === 'snake' || e.kind === 'scorpion' ? .28 : .21; ctx.scale(enemyScale, enemyScale); }
    ...
    if (e.kind === 'knightGuard') { drawKnightGuard(ctx,e,now,attacking);ctx.restore();if(e.stunnedUntil>now){ctx.fillStyle='#ffe56b';ctx.font='bold 14px monospace';ctx.fillText('★ ★ ★',e.x-7,e.y-22);}return; }
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if (keys.current.has('KeyE') && nearExit || keys.current.has('Period') && secondNearExit) {
        ...
        if(level===26&&castleGuardOutcome!=='peace'&&!(castleGuardOutcome==='fight'&&!enemies.current.some((enemy)=>enemy.kind==='knightGuard'))){if(castleGuardOutcome==='fight')setMessage('Сначала победи двух рыцарей-стражей!');else setCastleGuardDialogueOpen(true);}

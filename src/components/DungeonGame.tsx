...
export function DungeonGame({ paused = false, enemyMultiplier = 1, startingCoins = 0, oneHitBoss = false, startingLevel: requestedStartingLevel, profileName, players = 1, playerClass = 'knight', playerClass2 = 'knight', initialSave, tutorial = false, merchantMode = false, mobileControls = false, equippedSkin = 'default', equippedPet = null, travelToLevel, saveRequest = 0, onSaveSnapshot, onVictory, onShopOpenChange, networkRole = null, remotePosition = null, onNetworkPosition, remoteGameState = null, onNetworkGameState, keyBindings = DEFAULT_KEY_BINDINGS }: { paused?: boolean; enemyMultiplier?: number; startingCoins?: number; oneHitBoss?: boolean; startingLevel?: number | null; profileName: string; players?: 1 | 2; playerClass?: PlayerClass; playerClass2?: PlayerClass; initialSave?: GameSave | null; tutorial?: boolean; merchantMode?: boolean; mobileControls?: boolean; equippedSkin?: string; equippedPet?: 'cat'|'crocodile'|'pug'|'capybara'|null; travelToLevel?: number | null; saveRequest?: number; onSaveSnapshot?: (save: GameSave) => void; onVictory?: (level: number) => void; onShopOpenChange?: (open: boolean) => void; networkRole?: 'host' | 'guest' | null; remotePosition?: (Point & { fx?: number; fy?: number; moving?: boolean; attacking?:boolean }) | null; onNetworkPosition?: (position: Point & { fx?: number; fy?: number; moving?: boolean; attacking?:boolean }) => void; remoteGameState?: NetworkGameState | null; onNetworkGameState?: (state: NetworkGameState) => void; keyBindings?: KeyBindings }) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if(equippedPet){const pet=petPosition.current,valid=enemies.current.filter((enemy)=>enemy.hp>0&&!enemy.hiddenInRock&&enemy.kind!=='mudPile'),target=valid.reduce<Enemy|null>((best,enemy)=>!best||Math.hypot(enemy.x-pet.x,enemy.y-pet.y)<Math.hypot(best.x-pet.x,best.y-pet.y)?enemy:best,null),follow=target&&Math.hypot(target.x-pet.x,target.y-pet.y)<230?target:{x:p.x-34,y:p.y+18},px=follow.x-pet.x,py=follow.y-pet.y,pd=Math.max(1,Math.hypot(px,py));if(pd>24){pet.x+=px/pd*Math.min(pd,3.2*dt);pet.y+=py/pd*Math.min(pd,3.2*dt);}if(target&&pd<43&&now>=petAttackReadyAt.current){const damage=equippedPet==='cat'?1:equippedPet==='crocodile'?2:equippedPet==='pug'?3:4;target.hp-=damage;target.flash=12;if(equippedPet==='capybara')target.stunnedUntil=Math.max(target.stunnedUntil,now+1200);petAttackReadyAt.current=now+1200;petAttackUntil.current=now+280;}}

...
function drawScene(ctx: CanvasRenderingContext2D, map: ReturnType<typeof getLevel>, hero: Point, enemies: Enemy[], projectiles: Projectile[], superFists: SuperFist[], swordUltimates: SwordUltimate[], bowUltimates: BowUltimate[], staffUltimates: StaffUltimate[], glovesUltimates: GlovesUltimate[], waves: MagicWave[], sandTornadoes: SandTornado[], tombs: Tomb[], now: number, openedChests: number[], chestDrops: LootDrop[], attackProgress: number, loot: Point | null, droppedItem: Weapon | null, level: number, weapon: Weapon | null, weapon2: Weapon | null, armor: Weapon | null, facing: Point, moving: boolean, health: number, superReloading: boolean, profileName: string, secondHero: Point | null, secondFacing: Point, secondMoving: boolean, secondHealth: number, superReloading2: boolean, attackProgress2: number, armor2: Weapon | null, skin: HeroSkin, skin2: HeroSkin, explored: Point[], mobileControls = false, cameraFocus: Point = hero, castleGuardVisible = true) {
  ...
  enemies.forEach((e) => {
    ...
    if(boss&&level===30){drawKnightGuard(ctx,e,now,attacking);ctx.restore();return;}
    ...
    const loop = (now: number) => {
      ...
      enemies.current.forEach((e) => {
        ...
        if(level===30&&e.kind==='boss'){
          ...
          if(distance<=224&&e.attackUntil<=now&&now>=(e.nextShotAt??0)&&(e.hp>e.maxHp/2||e.bossAttack==='slam'||!e.bossAttack)){e.nextShotAt=now+10000;e.nextSummonAt=Math.max(e.nextSummonAt??0,now+10000);e.attackUntil=now+1000;e.bossAttack='spin';const hitFirst=health>0&&Math.hypot(p.x+12-e.x,p.y+14-e.y)<=224,hitSecond=players===2&&health2>0&&Math.hypot(p2.x+12-e.x,p2.y+14-e.y)<=224;if(hitFirst||hitSecond)damageHero(7,'Хранитель замка раскрутил огромный топор и нанёс 7 HP урона!',!hitFirst&&hitSecond);setMessage('Хранитель вращает топор длиной семь клеток — следующей будет атака по земле!');}
          ...
          if(e.hp<=e.maxHp/2&&now>=(e.nextContactAt??0)){const length=Math.max(1,distance);sandTornadoes.current.push({x:e.x,y:e.y-18,vx:ex/length*5.6,vy:ey/length*5.6,damage:4,until:Number.POSITIVE_INFINITY,style:'voidSaw'});e.nextContactAt=now+2400;setMessage('Обезглавленный хранитель запустил вечную чёрно-фиолетовую пилу!');}

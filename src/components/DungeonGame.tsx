...
type PetKind='cat'|'crocodile'|'pug'|'capybara';
function drawTinyPixelPetOverlay(ctx:CanvasRenderingContext2D,pet:PetKind,point:Point,camera:Point,attacking:boolean,now:number,facing=1,moving=false){const zoom=.5,viewW=640/zoom,viewH=360/zoom,cx=Math.round((camera.x+12-viewW/2)/2)*2,cy=Math.round((camera.y+14-viewH/2)/2)*2,cell=pet==='capybara'?2.2:1.52,jump=moving?(pet==='capybara'?Math.abs(Math.sin(now/150))*18:Math.abs(Math.sin(now/190))*7):attacking?5:0,lunge=attacking?16:0,color=(x:number,y:number):string|null=>{const limbColor=pet==='cat'?'#22252b':pet==='crocodile'?'#386335':pet==='pug'?'#a8734d':'#85583d',handColor=pet==='cat'?'#414650':pet==='crocodile'?'#75a956':pet==='pug'?'#d2a06d':'#bb8457',walking=moving&&Math.floor(now/140)%2===0,armPixel=(x>=3&&x<=5&&y>=11&&y<=15)||(attacking?x>=19&&x<=24&&y>=9&&y<=11:x>=21&&x<=23&&y>=10&&y<=14),legPixel=(x>=7&&x<=10&&y>=17+(walking?0:1)&&y<=19)||(x>=15&&x<=18&&y>=17+(walking?1:0)&&y<=19);if(armPixel||legPixel)return y===15||y===19?handColor:limbColor;if(pet==='cat'){if((x-8)*(x-8)+(y-13)*(y-13)<43||(x-16)*(x-16)*.7+(y-8)*(y-8)<38||y<6&&((x>=12&&x<=15-y/2)||(x>=19+y/2&&x<=22)))return y===8&&(x===15||x===19)?'#75f184':(x+y)%5===0?'#393d45':'#17191d';if(x<8&&y>=10&&y<=14&&Math.abs(y-(14-x/2))<2)return'#202329';}if(pet==='crocodile'){if(x>=3&&x<=22&&y>=8&&y<=15||x>=15&&x<=24&&y>=6&&y<=13||x<6&&y>=10-x/3&&y<=14+x/4)return y===8&&x>=18&&x<=21?'#e8ef9b':y===14&&x>=16&&x%2===0?'#fff0cf':(x+y)%6===0?'#79aa54':'#416f39';}if(pet==='pug'){if((x-9)*(x-9)*.65+(y-13)*(y-13)<45||(x-17)*(x-17)+(y-9)*(y-9)<48)return y>=10&&x>=18?'#5a3b32':y===8&&(x===15||x===19)?'#171313':(x+y)%6===0?'#e2b77e':'#bd8656';if(y<=4&&(x-17)*(x-17)+(y-4)*(y-4)*.7<33)return(x+y)%4===0?'#226e35':'#4dac50';}if(pet==='capybara'){if((x-10)*(x-10)*.5+(y-13)*(y-13)<50||(x-19)*(x-19)+(y-9)*(y-9)<43)return y>=10&&x>=21?'#6c4934':y===8&&x===19?'#171311':(x+y)%7===0?'#c89560':'#9b6845';if((x-18)*(x-18)+(y-3)*(y-3)<14)return(x+y)%4===0?'#ffbd3e':'#ed7e14';if(y===0&&x>=18&&x<=20)return'#4f983f';}return null;};ctx.save();ctx.imageSmoothingEnabled=false;ctx.scale(zoom,zoom);ctx.translate(-cx,-cy);ctx.translate(Math.round(point.x/cell)*cell+lunge*facing,Math.round((point.y-jump)/cell)*cell);if(facing<0){ctx.translate(30*cell,0);ctx.scale(-1,1);}ctx.save();ctx.translate(15*cell,20*cell+jump);ctx.fillStyle=pet==='cat'?'rgba(18,20,27,.5)':pet==='crocodile'?'rgba(12,25,13,.48)':pet==='pug'?'rgba(35,22,15,.46)':'rgba(26,17,12,.5)';ctx.beginPath();ctx.ellipse(0,0,(attacking?16:13)*cell,3.2*cell,0,0,Math.PI*2);ctx.fill();ctx.restore();for(let y=0;y<20;y++)for(let x=0;x<30;x++){const fill=color(Math.floor(x*25/30),y);if(fill){ctx.fillStyle='rgba(15,10,9,.42)';ctx.fillRect(x*cell+cell*.32,y*cell+cell*.45,cell,cell);}}for(let y=0;y<20;y++)for(let x=0;x<30;x++){const fill=color(Math.floor(x*25/30),y);if(fill){ctx.fillStyle=fill;ctx.fillRect(x*cell,y*cell,cell,cell);if((x+y)%7===0){ctx.fillStyle='rgba(255,244,198,.2)';ctx.fillRect(x*cell,y*cell,cell,cell*.28);}}}ctx.restore();}
...
export type Enemy = Point & { kind: EnemyKind; hp: number; maxHp: number; flash: number; attackUntil: number; stunnedUntil: number; color: string; power: number; speed: number; leapStarted: number; leapUntil: number; leapTargetX: number; leapTargetY: number; nextLeapAt: number; hiddenInRock?: boolean; nextShotAt?: number; nextSummonAt?: number; nextContactAt?: number; playerWasInSummonRadius?: boolean; revived?: boolean; reviveFlashUntil?: number; swallowUntil?: number; swallowedPlayer?: 1 | 2; swallowDamageDone?: boolean; thrownStarted?: number; thrownUntil?: number; thrownStartX?: number; thrownStartY?: number; thrownTargetX?: number; thrownTargetY?: number; bossAttack?: 'spin' | 'slam' | 'sword' | 'finalBoss'; finalAttack?: 1|2|3|4|5; finalAttackStarted?: number; finalAttackDir?: number };
export type NetworkGameState = { sender: 'host' | 'guest'; level: number; map?: ReturnType<typeof getLevel>; enemies: Enemy[]; openedChests: number[]; chestDrops: LootDrop[]; loot: Point | null; droppedItem: Weapon | null; localWeapon: Weapon | null; attacking: boolean; sentAt: number };
...
type SandTornado = Point & { vx: number; vy: number; damage: number; until: number; style?: 'sand' | 'ice' | 'iceRing' | 'iceSpiritDrop' | 'iceLarge' | 'iceShard' | 'frogTongue' | 'snake' | 'spear' | 'blood' | 'groundRift' | 'voidSaw' | 'bossSword' | 'voidSword' | 'fallingSword' | 'hellSawLarge' | 'hellSawSmall' | 'hellSpike' | 'hellMeteor' | 'hellShard' | 'bossShard'; impactAt?: number; launchAt?: number; flightSpeed?: number; split?: boolean; source?: Enemy; startedAt?: number; nextEmissionAt?: number; startX?: number; startY?: number; targetX?: number; targetY?: number; tongueDx?: number; tongueDy?: number; tongueLength?: number; hitPlayer1?: boolean; hitPlayer2?: boolean; hitCooldownUntil?: number };
...
function drawKnightGuard(ctx:CanvasRenderingContext2D,e:Enemy,now:number,attacking:boolean){
  const stride=Math.sin(now/120+e.x*.03)*4,sword=e.kind==='knightSword'||e.bossAttack==='sword',boss=e.kind==='boss';
  const headless=boss&&e.hp<=e.maxHp/2;ctx.fillStyle='rgba(0,0,0,.32)';ctx.beginPath();ctx.ellipse(64,111,39,9,0,0,Math.PI*2);ctx.fill();ctx.fillStyle=e.flash>0?'#fff':'#aebdca';ctx.fillRect(38,59,53,43);if(!headless){ctx.fillStyle='#252c35';ctx.fillRect(40,22,49,43);ctx.fillStyle=e.flash>0?'#fff':'#aebdca';ctx.fillRect(44,19,41,39);ctx.fillStyle='#dce7ec';ctx.fillRect(48,23,33,7);ctx.fillStyle='#202833';ctx.fillRect(48,36,33,8);ctx.fillStyle='#f2c15d';ctx.fillRect(54,38,6,4);ctx.fillRect(70,38,6,4);ctx.fillStyle='#8c263b';ctx.fillRect(59,7,12,18);ctx.fillRect(52,10,26,8);}else{ctx.fillStyle='#251b28';ctx.fillRect(54,54,21,8);ctx.fillStyle='#9a3dde';ctx.fillRect(59,55,11,3);}ctx.fillStyle='#6d7884';ctx.fillRect(27,62,13,37);ctx.fillRect(90,62,13,37);ctx.fillStyle='#843245';ctx.fillRect(43,70,43,27);ctx.fillStyle='#d4b354';ctx.fillRect(43,68,43,6);ctx.fillStyle='#343b46';ctx.fillRect(43,97+stride,18,14);ctx.fillRect(70,97-stride,18,14);
  const giantSpin=boss&&attacking&&e.bossAttack==='spin',hideThrownSword=sword&&boss&&now<(e.thrownUntil??0);if(!hideThrownSword){ctx.save();if(giantSpin){ctx.translate(64,66);ctx.rotate(now/75);ctx.translate(12,0);ctx.rotate(Math.PI/2);}else{const slamProgress=boss&&attacking&&e.bossAttack==='slam'?Math.max(0,Math.min(1,(e.attackUntil-now)/850)):0;ctx.translate(101,66);ctx.rotate(sword?(attacking?-1.15:.25):(slamProgress>.45?-1.45:.55));}
  const axeLength=giantSpin?184:78,axeHeadY=giantSpin?axeLength-5:0;ctx.fillStyle='#634126';ctx.fillRect(-5,-4,10,sword?35:axeLength);ctx.fillStyle='#becbd0';if(sword){ctx.fillRect(-7,-49,14,50);ctx.fillStyle='#f3fbff';ctx.fillRect(-4,-45,4,39);ctx.fillStyle='#d4b354';ctx.fillRect(-15,-3,30,7);}else{ctx.beginPath();ctx.moveTo(-5,axeHeadY-18);ctx.lineTo(-28,axeHeadY-8);ctx.lineTo(-22,axeHeadY+13);ctx.lineTo(5,axeHeadY+8);ctx.closePath();ctx.fill();ctx.fillStyle='#f3fbff';ctx.fillRect(-21,axeHeadY-6,14,4);}ctx.restore();}
}

function drawVoidHammer(ctx:CanvasRenderingContext2D,now:number){ctx.save();ctx.translate(64,66);ctx.rotate(now/75+Math.PI);ctx.translate(12,0);ctx.rotate(Math.PI/2);const length=184;ctx.fillStyle='#17111d';ctx.fillRect(-6,-4,12,length);ctx.strokeStyle='#8738c4';ctx.lineWidth=2;ctx.strokeRect(-6,-4,12,length);ctx.fillStyle='#24152f';ctx.fillRect(-23,length-16,46,27);ctx.fillStyle='#6d2396';ctx.fillRect(-20,length-13,40,21);ctx.fillStyle='#c565f0';ctx.fillRect(-17,length-10,7,15);ctx.restore();}

function drawFinalBoss(ctx:CanvasRenderingContext2D,e:Enemy,now:number){ctx.save();ctx.translate(16,-80);const flicker=.9+Math.sin(now/85)*.1;ctx.globalAlpha=flicker;ctx.fillStyle='rgba(0,0,0,.55)';ctx.beginPath();ctx.ellipse(48,204,47,10,0,0,Math.PI*2);ctx.fill();const cloak=e.flash>0?'#fff':'#070908';ctx.fillStyle=cloak;ctx.beginPath();ctx.moveTo(29,56);ctx.lineTo(67,56);ctx.lineTo(84,74);ctx.lineTo(96,210);ctx.lineTo(66,198);ctx.lineTo(48,210);ctx.lineTo(30,198);ctx.lineTo(0,210);ctx.lineTo(12,74);ctx.closePath();ctx.fill();ctx.fillStyle=e.flash>0?'#fff':'#0b0d0c';ctx.beginPath();ctx.moveTo(12,74);ctx.lineTo(29,56);ctx.lineTo(38,198);ctx.lineTo(30,198);ctx.lineTo(0,210);ctx.closePath();ctx.fill();ctx.beginPath();ctx.moveTo(84,74);ctx.lineTo(67,56);ctx.lineTo(58,198);ctx.lineTo(66,198);ctx.lineTo(96,210);ctx.closePath();ctx.fill();ctx.fillStyle=e.flash>0?'#fff':'#191312';ctx.beginPath();ctx.moveTo(38,58);ctx.lineTo(58,58);ctx.lineTo(57,194);ctx.lineTo(48,210);ctx.lineTo(39,194);ctx.closePath();ctx.fill();ctx.fillStyle='#111312';ctx.beginPath();ctx.moveTo(25,20);ctx.lineTo(33,10);ctx.lineTo(63,10);ctx.lineTo(71,20);ctx.lineTo(71,52);ctx.lineTo(63,64);ctx.lineTo(33,64);ctx.lineTo(25,52);ctx.closePath();ctx.fill();ctx.fillStyle='#080a09';ctx.fillRect(33,18,30,38);ctx.fillStyle='#17110f';ctx.fillRect(33,47,30,9);ctx.fillStyle='#ff5138';ctx.shadowColor='#ff2b19';ctx.shadowBlur=10;ctx.fillRect(35,39,7,4);ctx.fillRect(54,39,7,4);ctx.shadowBlur=0;ctx.restore();}
...
function drawSandTornado(ctx: CanvasRenderingContext2D, tornado: SandTornado, now: number) {
  if(tornado.style==='hellSawLarge'||tornado.style==='hellSawSmall'){const radius=tornado.style==='hellSawLarge'?64:32;ctx.save();ctx.translate(tornado.x,tornado.y);ctx.rotate(now/45);ctx.fillStyle='#160b1d';ctx.beginPath();for(let i=0;i<32;i++){const a=i/32*Math.PI*2,r=i%2?radius*.72:radius;const px=Math.cos(a)*r,py=Math.sin(a)*r;i?ctx.lineTo(px,py):ctx.moveTo(px,py);}ctx.closePath();ctx.fill();ctx.strokeStyle='#dc43ff';ctx.lineWidth=4;ctx.stroke();ctx.fillStyle='#6a1b83';ctx.beginPath();ctx.arc(0,0,radius*.32,0,Math.PI*2);ctx.fill();ctx.fillStyle='#ff896b';ctx.fillRect(-radius*.08,-radius*.42,radius*.16,radius*.84);ctx.restore();return;}
  if(tornado.style==='hellSpike'){ctx.save();ctx.translate(tornado.x,tornado.y);ctx.rotate(Math.atan2(tornado.vy,tornado.vx));ctx.fillStyle='#260d25';ctx.beginPath();ctx.moveTo(29,0);ctx.lineTo(-22,-10);ctx.lineTo(-14,0);ctx.lineTo(-22,10);ctx.closePath();ctx.fill();ctx.strokeStyle='#f04467';ctx.lineWidth=3;ctx.stroke();ctx.restore();return;}
  if(tornado.style==='hellMeteor'){ctx.save();ctx.translate(tornado.x,tornado.y);if(now<(tornado.impactAt??0)){const pulse=30+Math.sin(now/70)*5;ctx.strokeStyle='#ff4b37';ctx.lineWidth=4;ctx.beginPath();ctx.arc(0,0,pulse,0,Math.PI*2);ctx.stroke();ctx.fillStyle='rgba(255,45,20,.2)';ctx.fill();ctx.fillStyle='#ffb02e';ctx.fillRect(-3,-50,6,35);}else{ctx.fillStyle='#ff4b22';ctx.beginPath();ctx.arc(0,0,31,0,Math.PI*2);ctx.fill();ctx.fillStyle='#34101c';ctx.fillRect(-17,-12,16,15);ctx.fillRect(6,-18,13,12);ctx.fillStyle='#ffcc4c';ctx.fillRect(-5,-23,8,9);}ctx.restore();return;}
  if(tornado.style==='hellShard'||tornado.style==='bossShard'){ctx.save();ctx.translate(tornado.x,tornado.y);ctx.rotate(Math.atan2(tornado.vy,tornado.vx));ctx.fillStyle=tornado.style==='bossShard'?'#ff4567':'#ff9c36';ctx.beginPath();ctx.moveTo(12,0);ctx.lineTo(-8,-5);ctx.lineTo(-4,0);ctx.lineTo(-8,5);ctx.closePath();ctx.fill();ctx.fillStyle='#fff0ac';ctx.fillRect(1,-2,6,3);ctx.restore();return;}
  if(tornado.style==='bossSword'||tornado.style==='voidSword'){if(now<(tornado.launchAt??0))return;ctx.save();ctx.translate(tornado.x,tornado.y);ctx.rotate(Math.atan2(tornado.vy,tornado.vx)+Math.PI/2+now/65);const voidBlade=tornado.style==='voidSword';ctx.fillStyle='#634126';ctx.fillRect(-5,-4,10,35);ctx.fillStyle=voidBlade?'#6d2396':'#becbd0';ctx.fillRect(-7,-49,14,50);ctx.fillStyle=voidBlade?'#e294ff':'#f3fbff';ctx.fillRect(-4,-45,4,39);ctx.fillStyle=voidBlade?'#27152e':'#d4b354';ctx.fillRect(-15,-3,30,7);ctx.strokeStyle=voidBlade?'#bb58eb':'#ffffff';ctx.lineWidth=2;ctx.strokeRect(-7,-49,14,50);ctx.restore();return;}
  if(tornado.style==='fallingSword'){ctx.save();ctx.translate(tornado.x,tornado.y);if(now<(tornado.impactAt??0)){ctx.fillStyle='rgba(110,35,150,.24)';ctx.beginPath();ctx.arc(0,0,18,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#c65cff';ctx.lineWidth=3;ctx.stroke();ctx.fillStyle='#fff';ctx.fillRect(-1,-11,2,22);ctx.fillRect(-11,-1,22,2);}else{ctx.fillStyle='#27152e';ctx.fillRect(-5,-4,10,35);ctx.fillStyle='#6d2396';ctx.fillRect(-7,-49,14,50);ctx.fillStyle='#e294ff';ctx.fillRect(-4,-45,4,39);ctx.fillStyle='#1b1020';ctx.fillRect(-15,-3,30,7);ctx.strokeStyle='#bb58eb';ctx.lineWidth=2;ctx.strokeRect(-7,-49,14,50);}ctx.restore();return;}
  if(tornado.style==='voidSaw'){ctx.save();ctx.translate(tornado.x,tornado.y);ctx.rotate(now/55);ctx.fillStyle='#120d18';ctx.beginPath();for(let i=0;i<20;i++){const a=i/20*Math.PI*2,r=i%2===0?32:25;const x=Math.cos(a)*r,y=Math.sin(a)*r;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);}ctx.closePath();ctx.fill();ctx.strokeStyle='#9a3dde';ctx.lineWidth=3;ctx.stroke();ctx.fillStyle='#592078';ctx.beginPath();ctx.arc(0,0,12,0,Math.PI*2);ctx.fill();ctx.fillStyle='#e294ff';ctx.fillRect(-3,-5,6,10);ctx.restore();return;}
  if(tornado.style==='groundRift'){ctx.save();ctx.translate(tornado.x,tornado.y);ctx.rotate(Math.atan2(tornado.vy,tornado.vx));ctx.fillStyle='rgba(20,11,9,.42)';ctx.fillRect(-25,-10,50,20);ctx.strokeStyle='#251612';ctx.lineWidth=10;ctx.beginPath();ctx.moveTo(-25,0);ctx.lineTo(-13,-5);ctx.lineTo(-3,4);ctx.lineTo(9,-4);ctx.lineTo(25,2);ctx.stroke();ctx.strokeStyle='#d04732';ctx.lineWidth=3;ctx.stroke();ctx.fillStyle='#7e7168';ctx.fillRect(-18,-10,7,5);ctx.fillRect(4,5,9,5);ctx.restore();return;}
...
function createEnemies(map: ReturnType<typeof getLevel>, multiplier = 1, _oneHitBoss = false): Enemy[] {
...
function drawScene(ctx: CanvasRenderingContext2D, map: ReturnType<typeof getLevel>, hero: Point, enemies: Enemy[], projectiles: Projectile[], superFists: SuperFist[], swordUltimates: SwordUltimate[], bowUltimates: BowUltimate[], staffUltimates: StaffUltimate[], glovesUltimates: GlovesUltimate[], waves: MagicWave[], sandTornadoes: SandTornado[], tombs: Tomb[], now: number, openedChests: number[], chestDrops: LootDrop[], attackProgress: number, loot: Point | null, droppedItem: Weapon | null, level: number, weapon: Weapon | null, weapon2: Weapon | null, armor: Weapon | null, facing: Point, moving: boolean, health: number, superReloading: boolean, profileName: string, secondHero: Point | null, secondFacing: Point, secondMoving: boolean, secondHealth: number, superReloading2: boolean, attackProgress2: number, armor2: Weapon | null, skin: HeroSkin, skin2: HeroSkin, explored: Point[], mobileControls = false, cameraFocus: Point = hero, castleGuardVisible = true) {
  ...
  if(level===27){ctx.fillStyle='#17191d';ctx.fillRect(32,32,map.worldWidth-64,map.worldHeight-64);const castlePath=()=>{ctx.beginPath();ROUTE_POINTS.forEach((point,index)=>index===0?ctx.moveTo(point.x,point.y):ctx.lineTo(point.x,point.y));};ctx.lineJoin='round';ctx.lineCap='round';castlePath();ctx.strokeStyle='#282c32';ctx.lineWidth=176;ctx.stroke();castlePath();ctx.strokeStyle='#565b63';ctx.lineWidth=142;ctx.stroke();castlePath();ctx.strokeStyle='#b89442';ctx.lineWidth=66;ctx.stroke();castlePath();ctx.strokeStyle='#7c2635';ctx.lineWidth=56;ctx.stroke();}
  ...
  map.walls.forEach((wall,wallIndex) => {
    ...
    if(level===27){ctx.fillStyle='#383d45';ctx.fillRect(wall.x,wall.y,wall.w,wall.h);ctx.strokeStyle='#737983';ctx.lineWidth=3;ctx.strokeRect(wall.x,wall.y,wall.w,wall.h);for(let y=wall.y+12;y<wall.y+wall.h;y+=16){ctx.strokeStyle='#25292f';ctx.beginPath();ctx.moveTo(wall.x,y);ctx.lineTo(wall.x+wall.w,y);ctx.stroke();}return;}
  ...
  const finalEncounter=enemies.find((enemy)=>enemy.kind==='boss'&&enemy.bossAttack==='finalBoss'),finalElapsed=finalEncounter?now-(finalEncounter.reviveFlashUntil??now):0;
  if(finalEncounter&&finalElapsed>=2800){ctx.save();ctx.beginPath();ctx.arc(320,336,306,0,Math.PI*2);ctx.clip();ctx.fillStyle='#3a090d';ctx.fillRect(0,0,map.worldWidth,map.worldHeight);ctx.fillStyle='#671016';for(let y=20;y<map.worldHeight;y+=54)for(let x=18;x<map.worldWidth;x+=68){ctx.fillRect(x+(y%3)*7,y,38,24);ctx.fillStyle='#a82418';ctx.fillRect(x+8,y+22,28,4);ctx.fillStyle='#671016';}ctx.fillStyle='rgba(255,72,25,.15)';for(let i=0;i<18;i++){ctx.beginPath();ctx.arc((i*83)%map.worldWidth,(i*127)%map.worldHeight,18+(i%3)*8,0,Math.PI*2);ctx.fill();}ctx.restore();ctx.save();ctx.shadowColor='#ff2d20';ctx.shadowBlur=22;ctx.strokeStyle='#5a1117';ctx.lineWidth=28;ctx.beginPath();ctx.arc(320,336,306,0,Math.PI*2);ctx.stroke();ctx.strokeStyle='#ff4b27';ctx.lineWidth=7;ctx.stroke();ctx.strokeStyle='#ffb23f';ctx.lineWidth=2;ctx.setLineDash([12,10]);ctx.stroke();ctx.restore();}
  enemies.forEach((e) => {
    ...
    ctx.save();
    if (boss) { if(level===30){const slamJump=attacking&&e.bossAttack==='slam'?Math.sin(Math.max(0,Math.min(1,(e.attackUntil-now)/850))*Math.PI)*34:0;ctx.translate(e.x-72+(hero.x-e.x)/distance*force,e.y-126-slamJump+(hero.y-e.y)/distance*force);ctx.scale(1.15,1.15);}else{ctx.translate(e.x - 122 + (hero.x - e.x) / distance * force, e.y - 213 + (hero.y - e.y) / distance * force);ctx.scale(1.9, 1.9);} }
    else { ctx.translate(e.x + 1 + (hero.x - e.x) / distance * force, e.y - 5 + (hero.y - e.y) / distance * force); const enemyScale = e.kind === 'goblin' || e.kind === 'monkey' || e.kind === 'nativeSpear' || e.kind === 'nativeClub' || e.kind === 'knightGuard' || e.kind === 'knightSword' || e.kind === 'knightHalberd' || e.kind === 'mummy' || e.kind === 'iceGolem' || e.kind === 'iceSpirit' || e.kind === 'frog' || e.kind === 'mudPile' || e.kind === 'mudMonster' ? .38 : e.kind === 'snake' || e.kind === 'scorpion' ? .28 : .21; ctx.scale(enemyScale, enemyScale); }
    ...
    if (e.kind === 'knightGuard' || e.kind === 'knightSword' || e.kind === 'knightHalberd') { drawKnightGuard(ctx,e,now,attacking);ctx.restore();if(e.stunnedUntil>now){ctx.fillStyle='#ffe56b';ctx.font='bold 14px monospace';ctx.fillText('★ ★ ★',e.x-7,e.y-22);}return; }
    if(boss&&level===30){if(e.bossAttack==='finalBoss')drawFinalBoss(ctx,e,now);else{drawKnightGuard(ctx,e,now,attacking);if(attacking&&e.bossAttack==='spin'&&e.hp<=e.maxHp/2)drawVoidHammer(ctx,now);}ctx.restore();return;}
  ...
  if(finalEncounter?.finalAttack===1){const angle=(finalEncounter.thrownTargetX??0)+(finalEncounter.finalAttackDir??1)*(now-(finalEncounter.finalAttackStarted??now))*.00038;ctx.save();ctx.translate(finalEncounter.x,finalEncounter.y-8);ctx.rotate(angle);ctx.strokeStyle='rgba(255,38,48,.35)';ctx.lineWidth=42;ctx.beginPath();ctx.moveTo(0,0);ctx.lineTo(700,0);ctx.stroke();ctx.strokeStyle='#ff3948';ctx.lineWidth=32;ctx.stroke();ctx.strokeStyle='#fff0a0';ctx.lineWidth=8;ctx.stroke();ctx.fillStyle='#220817';ctx.fillRect(-12,-14,40,28);ctx.restore();}
  if(finalEncounter?.finalAttack===2){const elapsed=now-(finalEncounter.finalAttackStarted??now),travel=Math.max(0,Math.min(1,(elapsed-250)/750)),slam=Math.max(0,Math.min(1,(elapsed-1000)/220)),handX=finalEncounter.x+(finalEncounter.leapTargetX-finalEncounter.x)*travel,raisedY=finalEncounter.y-105+(finalEncounter.leapTargetY-finalEncounter.y)*travel,handY=raisedY+slam*105;ctx.save();ctx.strokeStyle=elapsed<1000?'#ff6b76':'#ffb13b';ctx.lineWidth=3;ctx.setLineDash([7,6]);ctx.beginPath();ctx.arc(finalEncounter.leapTargetX,finalEncounter.leapTargetY,42,0,Math.PI*2);ctx.stroke();ctx.setLineDash([]);ctx.translate(handX,handY);ctx.fillStyle='rgba(255,45,68,.28)';ctx.beginPath();ctx.arc(0,8,43,0,Math.PI*2);ctx.fill();ctx.fillStyle='#271020';ctx.fillRect(-30,-18,60,48);for(let i=-2;i<=2;i++)ctx.fillRect(i*12-5,-50,10,38);ctx.fillStyle='#6f244e';ctx.fillRect(-23,-11,46,13);ctx.strokeStyle='#f04b64';ctx.lineWidth=4;ctx.strokeRect(-30,-18,60,48);ctx.restore();if(elapsed>=1100){const grow=Math.max(0,Math.min(1,(elapsed-1100)/330)),fade=Math.max(0,Math.min(1,(2500-elapsed)/400));ctx.save();ctx.globalAlpha=fade;ctx.translate(finalEncounter.leapTargetX,finalEncounter.leapTargetY);ctx.fillStyle='#240c24';ctx.beginPath();ctx.moveTo(0,-76*grow);ctx.lineTo(21*grow,13);ctx.lineTo(-21*grow,13);ctx.closePath();ctx.fill();ctx.strokeStyle='#f04467';ctx.lineWidth=4;ctx.stroke();ctx.fillStyle='#ff9a62';ctx.beginPath();ctx.moveTo(-3,-62*grow);ctx.lineTo(5,-8);ctx.lineTo(-5,-8);ctx.closePath();ctx.fill();ctx.restore();}}
  if(finalEncounter?.finalAttack===4){ctx.fillStyle='rgba(255,48,72,.13)';ctx.beginPath();ctx.arc(finalEncounter.leapTargetX,finalEncounter.leapTargetY,64,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#ff4567';ctx.lineWidth=4;ctx.stroke();}
  ...
  if(swallowedPlayer!==1){const squeeze=finalEncounter&&finalElapsed>=1650&&finalElapsed<3200?Math.max(0,Math.min(1,(finalElapsed-1650)/650)):0;ctx.save();ctx.translate(hero.x+13,hero.y+14);ctx.scale(1-squeeze*.55,1+squeeze*.08);ctx.translate(-hero.x-13,-hero.y-14);drawHero(ctx, hero, attackProgress, weapon, armor, facing, moving, now, health, profileName, false, swordUltimates.find((ultimate) => ultimate.owner === 1 && now < ultimate.until), bowUltimates.find((ultimate) => ultimate.owner === 1 && now < ultimate.rainStarted), glovesUltimates.find((ultimate) => ultimate.owner === 1 && ultimate.name.includes('титана') && !ultimate.landed), skin);ctx.restore();}
  ...
  if(finalEncounter&&finalElapsed<3200){if(finalElapsed<1650){ctx.fillStyle='#08050b';ctx.fillRect(finalEncounter.x-174,finalEncounter.y-118,280,55);ctx.strokeStyle='#b52fdf';ctx.lineWidth=4;ctx.strokeRect(finalEncounter.x-170,finalEncounter.y-114,272,47);ctx.fillStyle='#f4d9ff';ctx.font='bold 13px monospace';ctx.fillText('ТЕПЕРЬ Я САМ СРАЖУСЬ С ТОБОЙ!',finalEncounter.x-158,finalEncounter.y-85);}else{const fall=Math.max(0,(finalElapsed-2450)/750);ctx.fillStyle='#050208';ctx.beginPath();ctx.ellipse(hero.x+13,hero.y+31,35+fall*75,13+fall*25,0,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#a42bc8';ctx.lineWidth=5;ctx.stroke();ctx.save();ctx.translate(hero.x+13,hero.y-18+fall*120);ctx.fillStyle='#6f2a76';ctx.fillRect(-26,-8,52,44);for(let finger=0;finger<5;finger++)ctx.fillRect(-31+finger*14,-44-Math.sin(finger)*8,10,42);ctx.fillStyle='#c45ed0';ctx.fillRect(-20,-2,34,8);ctx.restore();ctx.fillStyle='rgba(0,0,0,.75)';ctx.beginPath();ctx.ellipse(finalEncounter.x+10,finalEncounter.y+60,45+fall*65,17+fall*28,0,0,Math.PI*2);ctx.fill();}}
  if(finalEncounter&&finalElapsed>=1650&&finalElapsed<3200){const squeeze=Math.max(0,Math.min(1,(finalElapsed-1650)/650)),drawClosingHand=(target:Point)=>{ctx.save();ctx.translate(target.x+13,target.y+14);ctx.fillStyle='#4b1858';ctx.fillRect(-28+squeeze*7,-11,56-squeeze*14,34);ctx.fillStyle='#a84fba';for(let finger=0;finger<4;finger++){const side=finger<2?-1:1,index=finger%2,offset=42-squeeze*(23+index*5);ctx.save();ctx.translate(side*offset,-21+index*20);ctx.rotate(side*(.55+squeeze*.7));ctx.fillRect(side<0?-8:0,-6,8,34);ctx.fillStyle='#d47de0';ctx.fillRect(side<0?-7:1,-5,3,20);ctx.restore();ctx.fillStyle='#a84fba';}ctx.strokeStyle='#e18bea';ctx.lineWidth=3;ctx.strokeRect(-25+squeeze*7,-8,50-squeeze*14,28);ctx.restore();};drawClosingHand(hero);if(secondHero)drawClosingHand(secondHero);}
...
export function DungeonGame({ paused = false, enemyMultiplier = 1, startingCoins = 0, oneHitBoss = false, startingLevel: requestedStartingLevel, profileName, players = 1, playerClass = 'knight', playerClass2 = 'knight', initialSave, tutorial = false, merchantMode = false, mobileControls = false, equippedSkin = 'default', equippedPet = null, travelToLevel, saveRequest = 0, onSaveSnapshot, onVictory, onShopOpenChange, networkRole = null, remotePosition = null, onNetworkPosition, remoteGameState = null, onNetworkGameState, keyBindings = DEFAULT_KEY_BINDINGS }: { paused?: boolean; enemyMultiplier?: number; startingCoins?: number; oneHitBoss?: boolean; startingLevel?: number | null; profileName: string; players?: 1 | 2; playerClass?: PlayerClass; playerClass2?: PlayerClass; initialSave?: GameSave | null; tutorial?: boolean; merchantMode?: boolean; mobileControls?: boolean; equippedSkin?: string; equippedPet?: 'cat'|'crocodile'|'pug'|'capybara'|null; travelToLevel?: number | null; saveRequest?: number; onSaveSnapshot?: (save: GameSave) => void; onVictory?: (level: number) => void; onShopOpenChange?: (open: boolean) => void; networkRole?: 'host' | 'guest' | null; remotePosition?: (Point & { fx?: number; fy?: number; moving?: boolean; attacking?:boolean }) | null; onNetworkPosition?: (position: Point & { fx?: number; fy?: number; moving?: boolean; attacking?:boolean }) => void; remoteGameState?: NetworkGameState | null; onNetworkGameState?: (state: NetworkGameState) => void; keyBindings?: KeyBindings }) {
  ...
  const petPosition=useRef<Point>({x:startingPoint.x-38,y:startingPoint.y+18});
  const petFacing=useRef(1);
  const petMoving=useRef(false);
  const petStuckFrames=useRef(0);
  const petAttackReadyAt=useRef(0);
  const petAttackUntil=useRef(0);
  const pugMedkitRegion=useRef(-1);
  ...
  const pendingGuestTutorialChest = useRef<number|null>(null);
  const handledSaveRequest = useRef(saveRequest);
  const nextFootstepAt = useRef(0);
  const nextNetworkSendAt = useRef(0);
  const lastNetworkLevel = useRef(startingLevel);
  const guestDamageSnapshot = useRef<Array<{kind:EnemyKind;x:number;y:number;hp:number}>>([]);
  ...
  const [inventory, setInventory] = useState<Weapon[]>(initialSave?.inventory ?? (oneGameLoadout ? [ONE_GAME_SWORD, ONE_GAME_ARMOR] : permanentStarterSword ? [ONE_GAME_SWORD] : []));
  ...
  const [weapon, setWeapon] = useState<Weapon | null>(initialSave?.weapon ?? (permanentStarterSword ? ONE_GAME_SWORD : null));
  ...
  useEffect(()=>{if(equippedPet!=='pug'||level<=0)return;const region=Math.floor((level-1)/6);if(pugMedkitRegion.current===region)return;pugMedkitRegion.current=region;setMedkits((count)=>Math.min(5,count+1));if(players===2)setMedkits2((count)=>Math.min(5,count+1));setMessage('Арбузный мопс принёс аптечку для этой локации!');},[equippedPet,level,players]);
  const [dead, setDead] = useState(false);
  const [victory, setVictory] = useState(false);
  const [finaleStage, setFinaleStage] = useState(-1);
  ...
  useEffect(() => { if (!remotePosition || !networkRole) return; const remote = networkRole === 'host' ? hero2.current : hero.current; const moved = Math.hypot(remote.x - remotePosition.x, remote.y - remotePosition.y) > .4; remotePositionTarget.current={x:remotePosition.x,y:remotePosition.y};if (remotePosition.fx !== undefined && remotePosition.fy !== undefined) { const direction = networkRole === 'host' ? facing2 : facing; direction.current = { x: remotePosition.fx, y: remotePosition.fy }; } if(remotePosition.attacking){if(networkRole==='host')attackUntil2.current=performance.now()+190;else attackUntil.current=performance.now()+190;}if (moved || remotePosition.moving) remoteMovingUntil.current = performance.now() + 220; }, [networkRole, remotePosition]);
  useEffect(()=>{if(networkRole!=='guest'||!remoteGameState||remoteGameState.sender!=='host')return;if(remoteGameState.level!==level&&remoteGameState.map){const nextMap=remoteGameState.map,start=nextMap.round?{x:35,y:322}:getRouteStart();currentMap.current=nextMap;hero.current={...start};hero2.current=nextMap.round?{x:65,y:322}:{...start,y:start.y+32};remotePositionTarget.current=null;projectiles.current=[];superFists.current=[];swordUltimates.current=[];bowUltimates.current=[];staffUltimates.current=[];glovesUltimates.current=[];waves.current=[];sandTornadoes.current=[];}const pending=guestDamageSnapshot.current.filter((hit)=>{const server=remoteGameState.enemies.find((enemy)=>enemy.kind===hit.kind&&Math.hypot(enemy.x-hit.x,enemy.y-hit.y)<110);return Boolean(server&&server.hp>hit.hp);});enemies.current.forEach((local)=>{const server=remoteGameState.enemies.find((enemy)=>enemy.kind===local.kind&&Math.hypot(enemy.x-local.x,enemy.y-local.y)<80);if(server&&local.hp<server.hp&&!pending.some((hit)=>hit.kind===local.kind&&Math.hypot(hit.x-local.x,hit.y-local.y)<40))pending.push({kind:local.kind,x:local.x,y:local.y,hp:local.hp});});guestDamageSnapshot.current=pending;},[level,networkRole,remoteGameState]);
  useEffect(()=>{if(networkRole!=='host'||!remoteGameState||remoteGameState.sender!=='guest'||remoteGameState.level===level||!remoteGameState.map)return;const nextMap=remoteGameState.map,start=nextMap.round?{x:35,y:322}:getRouteStart();currentMap.current=nextMap;hero.current={...start};hero2.current=nextMap.round?{x:65,y:322}:{...start,y:start.y+32};remotePositionTarget.current=null;projectiles.current=[];superFists.current=[];swordUltimates.current=[];bowUltimates.current=[];staffUltimates.current=[];glovesUltimates.current=[];waves.current=[];sandTornadoes.current=[];},[level,networkRole,remoteGameState]);
  ...
  useEffect(()=>{if(!networkRole||!remoteGameState||remoteGameState.sender===networkRole)return;const incoming=remoteGameState.localWeapon??null,sameWeapon=(current:Weapon|null)=>current?.name===incoming?.name&&current?.damage===incoming?.damage&&current?.type===incoming?.type;if(networkRole==='host'){if(!sameWeapon(weapon2Ref.current)){setWeapon2(incoming);weapon2Ref.current=incoming;}if(remoteGameState.attacking)attackUntil2.current=performance.now()+210;}else{if(!sameWeapon(weaponRef.current)){setWeapon(incoming);weaponRef.current=incoming;}if(remoteGameState.attacking)attackUntil.current=performance.now()+210;guestDamageSnapshot.current.forEach((hit)=>{const target=enemies.current.filter((enemy)=>enemy.kind===hit.kind).sort((a,b)=>Math.hypot(a.x-hit.x,a.y-hit.y)-Math.hypot(b.x-hit.x,b.y-hit.y))[0];if(target&&Math.hypot(target.x-hit.x,target.y-hit.y)<110)target.hp=Math.min(target.hp,hit.hp);});}},[networkRole,remoteGameState]);
  ...
  useEffect(()=>{const pending=pendingGuestTutorialChest.current;if(networkRole!=='guest'||!remoteGameState||pending===null)return;if(remoteGameState.openedChests.includes(pending))pendingGuestTutorialChest.current=null;setOpenedChests((current)=>current.includes(pending)?current:[...current,pending]);},[networkRole,remoteGameState]);
  ...
  useEffect(() => {
    const down = (event: KeyboardEvent) => {
      ...
      if(networkRole==='host'&&['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Quote','Comma','Semicolon','Period','KeyL'].includes(event.code)){event.preventDefault();return;}
      ...
      if ((event.code === 'KeyQ' || players === 2 && networkRole===null && event.code === 'Comma') && !event.repeat) {
        const now = performance.now(); const secondCaster = players === 2 && (networkRole===null&&event.code === 'Comma'||networkRole==='guest'&&event.code==='KeyQ'); if (secondCaster ? health2 <= 0 : health <= 0) return; const cooldown = secondCaster ? superReadyAt2 : superReadyAt; if (now < cooldown.current) return;
      ...
      if (players === 2 && (networkRole===null&&event.code === 'Semicolon'||networkRole==='guest'&&event.code==='Space') && !event.repeat) {
  ...
  useEffect(() => { if (finaleStage < 0 || finaleStage >= 5) return; const durations=[7000,7000,7000,8000,9000];const timer=window.setTimeout(()=>setFinaleStage((stage)=>stage+1),durations[finaleStage]);return()=>window.clearTimeout(timer); }, [finaleStage]);
  ...
  const dropInventoryItem=(item:Weapon,index:number)=>{if(loot){setMessage('Сначала подбери предмет, который уже лежит на земле.');return;}const second=inventoryOwner===2,owner=second?hero2.current:hero.current;if(second){setInventory2((items)=>items.filter((_,itemIndex)=>itemIndex!==index));if(weapon2Ref.current===item){setWeapon2(null);weapon2Ref.current=null;}if(armor2===item)setArmor2(null);}else{setInventory((items)=>items.filter((_,itemIndex)=>itemIndex!==index));if(weaponRef.current===item){setWeapon(null);weaponRef.current=null;}if(armor===item)setArmor(null);}setDroppedItem(item);setLoot({x:owner.x+18,y:owner.y+28});setShowInventory(false);keys.current.clear();setMessage(`Игрок ${inventoryOwner} выбросил «${item.name}» на землю.`);};
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      const map = currentMap.current; const plantHitboxes = getPlantHitboxes(map); const solidHitboxes = [...map.walls, ...plantHitboxes]; const dt = Math.min((now - last) / 16.67, 2); last = now; const p = hero.current; const crocHpBefore=equippedPet==='crocodile'?new Map(enemies.current.map((enemy)=>[enemy,enemy.hp])):null; const baseSpeed = 2.25 * dt * (equippedPet==='cat'?1.05:1); const speed = now < slowedUntil.current ? baseSpeed / 1.5 : baseSpeed; const speed2 = now < slowedUntil2.current ? baseSpeed / 1.5 : baseSpeed;
      ...
      if(networkRole&&onNetworkPosition&&now>=nextNetworkSendAt.current){nextNetworkSendAt.current=now+50;const local=networkRole==='guest'?p2:p;const localFacing=networkRole==='guest'?facing2.current:facing.current;const localMoving=networkRole==='guest'?Boolean(dx2||dy2):Boolean(dx||dy),localAttacking=now<(networkRole==='guest'?attackUntil2:attackUntil).current;onNetworkPosition({x:local.x,y:local.y,fx:localFacing.x,fy:localFacing.y,moving:localMoving,attacking:localAttacking});}
      if(networkRole&&onNetworkGameState&&now>=nextNetworkStateAt.current){nextNetworkStateAt.current=now+(networkRole==='host'?140:100);const levelChanged=lastNetworkLevel.current!==level,includeMap=levelChanged||networkRole==='host'&&now>=nextNetworkMapAt.current;lastNetworkLevel.current=level;if(includeMap)nextNetworkMapAt.current=now+3000;const localWeapon=networkRole==='guest'?weapon2Ref.current:weaponRef.current,attacking=now<(networkRole==='guest'?attackUntil2:attackUntil).current;onNetworkGameState({sender:networkRole,level,map:includeMap?map:undefined,enemies:enemies.current.map((enemy)=>({...enemy})),openedChests:[...openedChests],chestDrops,loot,droppedItem,localWeapon,attacking,sentAt:Date.now()});}
      ...
      if(equippedPet){const pet=petPosition.current,valid=enemies.current.filter((enemy)=>enemy.hp>0&&!enemy.hiddenInRock&&enemy.kind!=='mudPile'&&!solidHitboxes.some((wall)=>segmentHitsRect({x:pet.x+15,y:pet.y+10},{x:enemy.x+14,y:enemy.y+14},wall))&&!map.carts.some((cart)=>segmentHitsRect({x:pet.x+15,y:pet.y+10},{x:enemy.x+14,y:enemy.y+14},{x:cart.x-5,y:cart.y,w:50,h:36}))),target=valid.reduce<Enemy|null>((best,enemy)=>!best||Math.hypot(enemy.x-pet.x,enemy.y-pet.y)<Math.hypot(best.x-pet.x,best.y-pet.y)?enemy:best,null),follow=target&&Math.hypot(target.x-pet.x,target.y-pet.y)<230?target:{x:p.x-34,y:p.y+18},px=follow.x-pet.x,py=follow.y-pet.y,pd=Math.max(1,Math.hypot(px,py));petMoving.current=false;if(pd>24){petMoving.current=true;if(Math.abs(px)>1)petFacing.current=px<0?-1:1;if(Math.hypot(p.x-pet.x,p.y-pet.y)>330){pet.x=p.x-38;pet.y=p.y+20;petStuckFrames.current=0;}const step=Math.min(pd,(target?4.1:3.5)*dt),nx=pet.x+px/pd*step,ny=pet.y+py/pd*step,blocked=(x:number,y:number)=>solidHitboxes.some((wall)=>x+16>wall.x&&x<wall.x+wall.w&&y+16>wall.y&&y<wall.y+wall.h)||map.carts.some((cart)=>x+16>cart.x-5&&x<cart.x+45&&y+16>cart.y&&y<cart.y+36),beforeX=pet.x,beforeY=pet.y;if(!blocked(nx,pet.y))pet.x=nx;if(!blocked(pet.x,ny))pet.y=ny;if(pet.x===beforeX&&pet.y===beforeY){petStuckFrames.current++;const sideX=-py/pd*step,sideY=px/pd*step;if(!blocked(pet.x+sideX,pet.y))pet.x+=sideX;if(!blocked(pet.x,pet.y+sideY))pet.y+=sideY;}else petStuckFrames.current=0;if(petStuckFrames.current>45){pet.x=p.x-38;pet.y=p.y+20;petStuckFrames.current=0;}}if(target&&pd<43&&now>=petAttackReadyAt.current){const damage=equippedPet==='cat'?1:equippedPet==='crocodile'?2:equippedPet==='pug'?3:4;target.hp-=damage;target.flash=12;if(equippedPet==='capybara')target.stunnedUntil=Math.max(target.stunnedUntil,now+1200);petAttackReadyAt.current=now+1200;petAttackUntil.current=now+280;}}
      ...
      enemies.current.forEach((e) => {
        ...
        if(level===30&&e.kind==='boss'){
          if((e.bossAttack as string)==='finalBoss'){
            ...
            if(e.finalAttack===1){const angle=(e.thrownTargetX??0)+(e.finalAttackDir??1)*(now-(e.finalAttackStarted??now))*.00038,hit=(h:Point)=>{const hx=h.x+12-e.x,hy=h.y+14-e.y,along=hx*Math.cos(angle)+hy*Math.sin(angle),side=Math.abs(hx*Math.sin(angle)-hy*Math.cos(angle));return along>0&&along<650&&side<16;};if(now>=(e.nextContactAt??0)&&(hit(p)||(players===2&&hit(p2)))){damageHero(6,'Адский луч нанёс 6 HP урона!',!hit(p)&&players===2&&hit(p2));e.nextContactAt=now+850;}}
            if(e.finalAttack===2&&now-(e.finalAttackStarted??now)>=1000&&now>=(e.nextContactAt??0)){const hit=(h:Point)=>Math.hypot(h.x+12-e.leapTargetX,h.y+14-e.leapTargetY)<=46;if(hit(p)||(players===2&&hit(p2))){damageHero(6,'Рука ударила по земле, и адский шип нанёс 6 HP!',!hit(p)&&players===2&&hit(p2));}e.nextContactAt=e.attackUntil+1;}
            return;
          }
          if(e.bossAttack==='finalBoss'){const elapsed=now-(e.reviveFlashUntil??now);if(elapsed<3200){const fall=Math.max(0,Math.min(1,(elapsed-2450)/750));keys.current.clear();mobileMove.current={x:0,y:0};p.x=225;p.y=350+fall*180;if(players===2){p2.x=190;p2.y=350+fall*180;}e.x=408;e.y=270+fall*180;return;}if(!e.revived){e.revived=true;p.x=210;p.y=430;if(players===2){p2.x=175;p2.y=430;}e.x=410;e.y=210;e.nextShotAt=now+1300;setMessage('Вы провалились в ад. Финальная битва начинается!');}if(distance<=85&&now>=(e.nextShotAt??0)){e.nextShotAt=now+2200;e.attackUntil=now+650;const hitFirst=health>0&&Math.hypot(p.x+12-e.x,p.y+14-e.y)<=85,hitSecond=players===2&&health2>0&&Math.hypot(p2.x+12-e.x,p2.y+14-e.y)<=85;if(hitFirst||hitSecond)damageHero(4,'Главный босс ударил адской энергией!',!hitFirst&&hitSecond);}if(e.attackUntil<=now&&distance>58){const step=e.speed*dt,moveX=e.x+ex/distance*step,moveY=e.y+ey/distance*step;if(Math.hypot(moveX-320,moveY-336)<275){e.x=moveX;e.y=moveY;}}return;}
          if(e.bossAttack==='sword'){if(now>=(e.nextShotAt??0)){const quarter=e.hp<=e.maxHp/4,half=e.hp<=e.maxHp/2,count=quarter?3:half?2:1,spacing=quarter?500:1000,flightSpeed=quarter?9.3:6.2;e.nextShotAt=now+7000;e.attackUntil=now+650;e.thrownUntil=now+(count-1)*spacing+3000;for(let swordIndex=0;swordIndex<count;swordIndex++){const launchAt=now+swordIndex*spacing;sandTornadoes.current.push({x:e.x,y:e.y-20,vx:0,vy:0,damage:7,launchAt,impactAt:launchAt+750,until:launchAt+3000,flightSpeed,style:swordIndex>0?'voidSword':'bossSword',source:e});}setMessage(quarter?'У босса осталось четверть HP — три ускоренных меча летят с интервалом 0,5 секунды!':half?'Страж бросает меч из руки, а через секунду отдельно наводит второй меч!':'Страж метнул меч прямо из руки — он вернётся обратно!');}if(e.hp<=e.maxHp/2&&now>=(e.nextSummonAt??0)){e.nextSummonAt=now+5000;for(let swordIndex=0;swordIndex<10;swordIndex++){const angle=Math.random()*Math.PI*2,radius=Math.sqrt(Math.random())*245;sandTornadoes.current.push({x:320+Math.cos(angle)*radius,y:336+Math.sin(angle)*radius,vx:0,vy:0,damage:2,impactAt:now+200,until:now+750,style:'fallingSword'});}setMessage('Через 0,2 секунды с потолка упадут десять чёрно-фиолетовых мечей!');}if(e.attackUntil<=now&&distance>46){const step=Math.max(.75,e.speed)*dt,moveX=e.x+ex/distance*step,moveY=e.y+ey/distance*step;if(Math.hypot(moveX-320,moveY-336)<275){e.x=moveX;e.y=moveY;}}return;}
          if(distance<=224&&e.attackUntil<=now&&now>=(e.nextShotAt??0)&&(e.hp>e.maxHp/2||e.bossAttack==='slam'||!e.bossAttack)){e.nextShotAt=now+10000;e.nextSummonAt=Math.max(e.nextSummonAt??0,now+10000);e.attackUntil=now+1000;e.bossAttack='spin';const secondHammer=e.hp<=e.maxHp/2,spinDamage=secondHammer?9:7,hitFirst=health>0&&Math.hypot(p.x+12-e.x,p.y+14-e.y)<=224,hitSecond=players===2&&health2>0&&Math.hypot(p2.x+12-e.x,p2.y+14-e.y)<=224;if(hitFirst||hitSecond)damageHero(spinDamage,secondHammer?'Основной топор нанёс 7 HP, а чёрно-фиолетовый молот ещё 2 HP!':'Хранитель замка раскрутил огромный топор и нанёс 7 HP урона!',!hitFirst&&hitSecond);setMessage(secondHammer?'Два оружия вращаются навстречу друг другу — следующей будет атака по земле!':'Хранитель вращает топор длиной семь клеток — следующей будет атака по земле!');}
          if(e.hp<=e.maxHp/2&&e.bossAttack==='spin'&&e.attackUntil<=now&&now>=(e.nextSummonAt??0)){e.nextSummonAt=now+10000;e.nextShotAt=Math.max(e.nextShotAt??0,now+10000);e.attackUntil=now+850;e.bossAttack='slam';for(let index=0;index<7;index++){const angle=index/7*Math.PI*2;sandTornadoes.current.push({x:e.x,y:e.y,vx:Math.cos(angle)*5.2,vy:Math.sin(angle)*5.2,damage:7,until:now+1600,style:'groundRift'});}setMessage('Хранитель подпрыгнул и ударил топором в пол — следующая атака через десять секунд!');}
          if(e.hp<=e.maxHp/2&&now>=(e.nextContactAt??0)&&sandTornadoes.current.filter((shot)=>shot.style==='voidSaw').length<7){const length=Math.max(1,distance);sandTornadoes.current.push({x:e.x,y:e.y-18,vx:ex/length*5.6,vy:ey/length*5.6,damage:4,until:Number.POSITIVE_INFINITY,style:'voidSaw'});e.nextContactAt=now+5000;setMessage('Обезглавленный хранитель запустил вечную чёрно-фиолетовую пилу!');}
      ...
      sandTornadoes.current = sandTornadoes.current.filter((tornado) => {
        const oldX=tornado.x,oldY=tornado.y;tornado.x+=tornado.vx*dt;tornado.y+=tornado.vy*dt;
        if(tornado.style==='hellSpike'){const start=tornado.startedAt??now,impact=tornado.impactAt??start+1800,retreat=tornado.launchAt??impact+3000,end=tornado.until,sx=tornado.startX??tornado.x,sy=tornado.startY??tornado.y,tx=tornado.targetX??tornado.x,ty=tornado.targetY??tornado.y;if(now<impact){const q=Math.max(0,Math.min(1,(now-start)/(impact-start)));tornado.x=sx+(tx-sx)*q;tornado.y=sy+(ty-sy)*q;}else if(now<retreat){tornado.x=tx;tornado.y=ty;}else{const q=Math.max(0,Math.min(1,(now-retreat)/(end-retreat)));tornado.x=tx+(sx-tx)*q;tornado.y=ty+(sy-ty)*q;}}
        if(tornado.style==='hellSawLarge'||tornado.style==='hellSawSmall'){const radius=tornado.style==='hellSawLarge'?64:32,dx=tornado.x-320,dy=tornado.y-336,d=Math.max(1,Math.hypot(dx,dy)),limit=306-radius;if(d>limit){const nx=dx/d,ny=dy/d,dot=tornado.vx*nx+tornado.vy*ny;tornado.vx-=2*dot*nx;tornado.vy-=2*dot*ny;tornado.x=320+nx*limit;tornado.y=336+ny*limit;}if(tornado.style==='hellSawLarge'&&now>=(tornado.nextEmissionAt??Infinity)){tornado.nextEmissionAt=(tornado.nextEmissionAt??now)+2500;const base=Math.atan2(tornado.vy,tornado.vx)+(Math.random()<.5?-1:1)*Math.PI/2;newIceShards.push({x:tornado.x,y:tornado.y,vx:Math.cos(base)*5.4,vy:Math.sin(base)*5.4,damage:6,until:tornado.until,style:'hellSawSmall'});}}
        if(tornado.style==='hellMeteor'&&now>=(tornado.impactAt??Infinity)&&tornado.split){tornado.split=false;for(let i=0;i<20;i++){const a=i/20*Math.PI*2+(i%2)*.08;newIceShards.push({x:tornado.x,y:tornado.y,vx:Math.cos(a)*(4.5+(i%4)*.45),vy:Math.sin(a)*(4.5+(i%4)*.45),damage:1,until:now+2100,style:'hellShard'});}}
        if(tornado.style==='bossSword'||tornado.style==='voidSword'){const source=tornado.source,speed=tornado.flightSpeed??6.2;if(!source||source.hp<=0)return false;if(now<(tornado.launchAt??0)){tornado.x=source.x;tornado.y=source.y-20;return true;}if(!tornado.startedAt){const firstDistance=health>0?Math.hypot(p.x+12-source.x,p.y+14-source.y):Infinity,secondDistance=players===2&&health2>0?Math.hypot(p2.x+12-source.x,p2.y+14-source.y):Infinity,target=secondDistance<firstDistance?p2:p,targetX=target.x+12-source.x,targetY=target.y+14-(source.y-20),targetDistance=Math.max(1,Math.hypot(targetX,targetY));tornado.x=source.x;tornado.y=source.y-20;tornado.vx=targetX/targetDistance*speed;tornado.vy=targetY/targetDistance*speed;tornado.startedAt=now;}if(now>=(tornado.impactAt??Infinity)){const returnX=source.x-tornado.x,returnY=source.y-20-tornado.y,returnDistance=Math.max(1,Math.hypot(returnX,returnY));if(returnDistance<24){if(tornado.style==='bossSword')source.thrownUntil=0;return false;}tornado.vx=returnX/returnDistance*speed;tornado.vy=returnY/returnDistance*speed;}}
        if(tornado.style==='voidSaw'){const firstGuardianAlive=enemies.current.some((enemy)=>level===30&&enemy.kind==='boss'&&enemy.hp>0&&enemy.bossAttack!=='sword');if(!firstGuardianAlive)return false;const dx=tornado.x-320,dy=tornado.y-336,distanceFromCenter=Math.max(1,Math.hypot(dx,dy)),arenaLimit=306;if(distanceFromCenter>arenaLimit){const nx=dx/distanceFromCenter,ny=dy/distanceFromCenter,dot=tornado.vx*nx+tornado.vy*ny;tornado.vx-=2*dot*nx;tornado.vy-=2*dot*ny;tornado.x=320+nx*arenaLimit;tornado.y=336+ny*arenaLimit;}const wall=map.walls.find((item)=>tornado.x+32>item.x&&tornado.x-32<item.x+item.w&&tornado.y+32>item.y&&tornado.y-32<item.y+item.h);if(wall){const hitX=oldX+32>wall.x&&oldX-32<wall.x+wall.w,hitY=oldY+32>wall.y&&oldY-32<wall.y+wall.h;if(!hitX)tornado.vx*=-1;else if(!hitY)tornado.vy*=-1;else{tornado.vx*=-1;tornado.vy*=-1;}tornado.x=oldX;tornado.y=oldY;}}
        ...
        const waitingToFall = Boolean(tornado.impactAt && now < tornado.impactAt && tornado.style !== 'bossSword' && tornado.style !== 'voidSword');
        ...
        const radius = tornado.style === 'hellSawLarge' ? 64 : tornado.style === 'hellSawSmall' ? 32 : tornado.style === 'hellMeteor' ? 31 : tornado.style === 'hellSpike' ? 13 : tornado.style === 'hellShard' || tornado.style === 'bossShard' ? 10 : tornado.style === 'voidSaw' ? 32 : tornado.style === 'bossSword' || tornado.style === 'voidSword' ? 14 : tornado.style === 'fallingSword' ? 18 : tornado.style === 'iceLarge' ? 25 : tornado.style === 'iceSpiritDrop' ? 16 : tornado.style === 'iceRing' ? 12 : tornado.style === 'iceShard' ? 13 : tornado.style === 'frogTongue' ? 8 : 22;
        const reusableHit=tornado.style==='voidSaw'||tornado.style==='bossSword'||tornado.style==='voidSword'||tornado.style==='hellSawLarge'||tornado.style==='hellSawSmall'||tornado.style==='hellSpike',canSawHit=!reusableHit||now>=(tornado.hitCooldownUntil??0);const hitFirst = canSawHit&&!waitingToFall && health > 0 && !tornado.hitPlayer1 && Math.hypot(p.x + 12 - tornado.x, p.y + 14 - tornado.y) < radius;
        const hitSecond = canSawHit&&!waitingToFall && players === 2 && health2 > 0 && !tornado.hitPlayer2 && Math.hypot(p2.x + 12 - tornado.x, p2.y + 14 - tornado.y) < radius;
        ...
        if(tornado.style==='bossSword'||tornado.style==='voidSword')return now<tornado.until;return now < tornado.until && tornado.x > 0 && tornado.x < map.worldWidth && tornado.y > 0 && tornado.y < map.worldHeight;
      });
      ...
      if(crocHpBefore)enemies.current.forEach((enemy)=>{const before=crocHpBefore.get(enemy);if(before!==undefined&&enemy.hp<before)enemy.hp-=((before-enemy.hp)*.1);});
      const defeated = enemies.current.filter((e) => e.hp <= 0);
      if(networkRole==='guest')defeated.forEach((enemy)=>{if(!guestDamageSnapshot.current.some((hit)=>hit.kind===enemy.kind&&Math.hypot(hit.x-enemy.x,hit.y-enemy.y)<40))guestDamageSnapshot.current.push({kind:enemy.kind,x:enemy.x,y:enemy.y,hp:0});});
      ...
      const multiplayerTutorialChest=level===0&&players===2;
      const nearChest = map.chests.findIndex((chest, index) => !openedChests.includes(index) && (!multiplayerTutorialChest||index===(secondInteracts?1:0)) && Math.hypot(chest.x + 24 - interactionHero.x, chest.y + 20 - interactionHero.y) < 75);
      if(multiplayerTutorialChest&&(keys.current.has('KeyE')||secondInteracts)&&nearChest>=0&&!choiceItem){const ownerClass=secondInteracts?playerClass2:playerClass,drop=getTutorialClassLoot(ownerClass);itemPicker.current=secondInteracts?2:1;if(networkRole==='guest')pendingGuestTutorialChest.current=nearChest;setChestDrops((drops)=>drops.map((current,index)=>index===nearChest?drop:current));setOpenedChests((current)=>current.includes(nearChest)?current:[...current,nearChest]);keys.current.clear();setChoiceItem(drop.item);setMessage(`Игрок ${secondInteracts?2:1} открыл свой учебный сундук: «${drop.item.name}».`);}
      if(networkRole==='guest'&&secondInteracts&&nearChest>=0&&!loot&&!multiplayerTutorialChest){const chest=map.chests[nearChest],drop=level===0?getTutorialClassLoot(playerClass2):chestDrops[nearChest];pendingGuestChest.current={index:nearChest,loot:{x:chest.x+8,y:chest.y+48},item:drop.item};}
      if (!multiplayerTutorialChest&&(keys.current.has('KeyE') || secondInteracts) && nearChest >= 0 && !loot) { const chest = map.chests[nearChest]; const ownerClass = secondInteracts ? playerClass2 : playerClass; let drop = level === 0 ? getTutorialClassLoot(ownerClass) : chestDrops[nearChest]; if (level === 0) setChestDrops((drops) => drops.map((current, index) => index === nearChest ? drop : current)); itemPicker.current = secondInteracts ? 2 : 1; if (!secondInteracts) keys.current.delete('KeyE'); setOpenedChests((current) => [...current, nearChest]); setDroppedItem(drop.item); setLoot({ x: chest.x + 8, y: chest.y + 48 }); if (level > 0 && level < 6 && Math.random() < .1) ambushAt.current = performance.now(); setMessage(`${drop.rarity.name} сундук: выпал предмет «${drop.item.name}»!`); }
  ...
  return <section className="game-shell">
    ...
    <div className="canvas-stage"><canvas ref={canvasRef} width={WIDTH} height={HEIGHT} />
      {level===26&&castleGuardDialogueOpen&&<CastleGuardDialogue onClose={()=>{keys.current.clear();mobileMove.current={x:0,y:0};setCastleGuardDialogueOpen(false);}} onOutcome={resolveCastleGuardDialogue}/>}
      ...
      {showInventory && <div className="inventory-panel"><div className="inventory-title"><div><small>РЮКЗАК · ИГРОК {inventoryOwner}</small><strong>{visibleInventory.length}/{visibleCapacity} МЕСТ</strong></div><button onClick={() => { keys.current.clear(); setShowInventory(false); }}>✕</button></div><div className="inventory-grid">{Array.from({ length: visibleCapacity }, (_, index) => { const item = visibleInventory[index]; return <button key={index} className={`inventory-slot ${item ? 'filled' : ''}`} onClick={() => item && equipInventoryItem(item)} title={item?.name}>{item ? <><span>{item.type === 'bow' ? '🏹' : item.type === 'staff' ? '🔮' : item.type === 'gloves' ? '✊' : item.type === 'armor' ? '🛡️' : '⚔️'}</span><small>{item.name}</small><b>{item.type === 'armor' ? `🛡${item.durability}` : `⚔${item.damage}`}</b><em className="inventory-drop-action" onClick={(event)=>{event.stopPropagation();dropInventoryItem(item,index);}}>БРОСИТЬ</em></> : <i>{index + 1}</i>}</button>; })}</div><div className="inventory-footer"><span>ОСКОЛКИ: {coins}</span><strong className="medkit-count">🧰 АПТЕЧКИ: {inventoryOwner === 2 ? medkits2 : medkits}/5</strong>{visibleCapacity < 30 ? <button onClick={upgradeInventory}>УЛУЧШИТЬ ДО {visibleCapacity === 10 ? 20 : 30} · {visibleCapacity === 10 ? 100 : 250}</button> : <b>МАКСИМУМ: 30</b>}</div></div>}

    ...
    const loop = (now: number) => {
      ...
      enemies.current.forEach((e) => {
        ...
        if(level===30&&e.kind==='boss'){
          ...
          if(distance<=224&&e.attackUntil<=now&&now>=(e.nextShotAt??0)){e.nextShotAt=now+10000;e.nextSummonAt=Math.max(e.nextSummonAt??0,now+10000);e.attackUntil=now+1000;e.bossAttack='spin';const hitFirst=health>0&&Math.hypot(p.x+12-e.x,p.y+14-e.y)<=224,hitSecond=players===2&&health2>0&&Math.hypot(p2.x+12-e.x,p2.y+14-e.y)<=224;if(hitFirst||hitSecond)damageHero(7,'Хранитель замка раскрутил огромный топор и нанёс 7 HP урона!',!hitFirst&&hitSecond);setMessage('Хранитель вращает топор длиной семь клеток — следующая атака через десять секунд!');}
          if(e.hp<=e.maxHp/2&&e.attackUntil<=now&&now>=(e.nextSummonAt??0)){e.nextSummonAt=now+10000;e.nextShotAt=Math.max(e.nextShotAt??0,now+10000);e.attackUntil=now+850;e.bossAttack='slam';for(let index=0;index<7;index++){const angle=index/7*Math.PI*2;sandTornadoes.current.push({x:e.x,y:e.y,vx:Math.cos(angle)*5.2,vy:Math.sin(angle)*5.2,damage:7,until:now+1600,style:'groundRift'});}setMessage('Хранитель подпрыгнул и ударил топором в пол — следующая атака через десять секунд!');}

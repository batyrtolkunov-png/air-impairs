...
function drawKnightGuard(ctx:CanvasRenderingContext2D,e:Enemy,now:number,attacking:boolean){
  const stride=Math.sin(now/120+e.x*.03)*4,sword=e.kind==='knightSword'||e.bossAttack==='idleSword',boss=e.kind==='boss';
  ...
  ctx.save();if(boss&&attacking&&e.bossAttack==='spin'){ctx.translate(64,66);ctx.rotate(now/75);ctx.translate(72,0);ctx.rotate(Math.PI/2);}else{const slamProgress=boss&&attacking&&e.bossAttack==='slam'?Math.max(0,Math.min(1,(e.attackUntil-now)/850)):0;ctx.translate(101,66);ctx.rotate(slamProgress>.45?-1.45:.55);}
  ctx.fillStyle='#634126';ctx.fillRect(-5,-4,10,sword?35:78);ctx.fillStyle='#becbd0';if(sword){ctx.fillRect(-7,-49,14,50);ctx.fillStyle='#f3fbff';ctx.fillRect(-4,-45,4,39);ctx.fillStyle='#d4b354';ctx.fillRect(-15,-3,30,7);}else{ctx.beginPath();ctx.moveTo(-5,-18);ctx.lineTo(-28,-8);ctx.lineTo(-22,13);ctx.lineTo(5,8);ctx.closePath();ctx.fill();ctx.fillStyle='#f3fbff';ctx.fillRect(-21,-6,14,4);}ctx.restore();
}
    ...
    const loop = (now: number) => {
      ...
      enemies.current.forEach((e) => {
        ...
        if(level===30&&e.kind==='boss'){
          ...
          if(distance<=128&&now>=(e.nextShotAt??0)){e.nextShotAt=now+2800;e.attackUntil=now+1000;e.bossAttack='spin';const hitFirst=health>0&&Math.hypot(p.x+12-e.x,p.y+14-e.y)<=128,hitSecond=players===2&&health2>0&&Math.hypot(p2.x+12-e.x,p2.y+14-e.y)<=128;if(hitFirst||hitSecond)damageHero(7,'Хранитель замка раскрутил длинный топор и нанёс 7 HP урона!',!hitFirst&&hitSecond);setMessage('Хранитель замка вращает длинный топор — отойди дальше четырёх клеток!');}
      ...
      const fallenCastleGuardian=enemies.current.find((e)=>level===30&&e.kind==='boss'&&e.hp<=0&&e.bossAttack!=='idleSword');
      if(fallenCastleGuardian){fallenCastleGuardian.hp=fallenCastleGuardian.maxHp;fallenCastleGuardian.flash=0;fallenCastleGuardian.attackUntil=0;fallenCastleGuardian.bossAttack='idleSword';fallenCastleGuardian.power=0;fallenCastleGuardian.speed=0;fallenCastleGuardian.x=320;fallenCastleGuardian.y=260;setMessage('Страж с топором повержен. На арену вышел второй огромный страж с мечом — пока он неподвижен.');}
      ...
      const nearJunglePortal=level===26&&p.x<100&&Math.abs(p.y+14-getRouteStart().y)<75;const secondNearJunglePortal=players===2&&level===26&&p2.x<100&&Math.abs(p2.y+14-getRouteStart().y)<75;

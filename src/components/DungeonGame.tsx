...
function drawSandTornado(ctx: CanvasRenderingContext2D, tornado: SandTornado, now: number) {
  if(tornado.style==='voidSaw'){ctx.save();ctx.translate(tornado.x,tornado.y);ctx.rotate(now/55);ctx.fillStyle='#120d18';ctx.beginPath();for(let i=0;i<16;i++){const a=i/16*Math.PI*2,r=i%2===0?10:7;const x=Math.cos(a)*r,y=Math.sin(a)*r;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);}ctx.closePath();ctx.fill();ctx.strokeStyle='#9a3dde';ctx.lineWidth=2;ctx.stroke();ctx.fillStyle='#592078';ctx.beginPath();ctx.arc(0,0,4,0,Math.PI*2);ctx.fill();ctx.fillStyle='#e294ff';ctx.fillRect(-1,-2,2,4);ctx.restore();return;}
    ...
    const loop = (now: number) => {
      ...
      enemies.current.forEach((e) => {
        ...
        if(level===30&&e.kind==='boss'){
          ...
          if(e.hp<=e.maxHp/2&&now>=(e.nextContactAt??0)){const length=Math.max(1,distance);sandTornadoes.current.push({x:e.x,y:e.y-18,vx:ex/length*5.6,vy:ey/length*5.6,damage:4,until:now+2600,style:'voidSaw'});e.nextContactAt=now+2400;setMessage('Обезглавленный хранитель запустил чёрно-фиолетовую пилу!');}
      ...
      sandTornadoes.current = sandTornadoes.current.filter((tornado) => {
        ...
        const radius = tornado.style === 'voidSaw' ? 10 : tornado.style === 'iceLarge' ? 25 : tornado.style === 'iceSpiritDrop' ? 16 : tornado.style === 'iceRing' ? 12 : tornado.style === 'iceShard' ? 13 : tornado.style === 'frogTongue' ? 8 : 22;

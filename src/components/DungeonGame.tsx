...
function drawScene(ctx: CanvasRenderingContext2D, map: ReturnType<typeof getLevel>, hero: Point, enemies: Enemy[], projectiles: Projectile[], superFists: SuperFist[], swordUltimates: SwordUltimate[], bowUltimates: BowUltimate[], staffUltimates: StaffUltimate[], glovesUltimates: GlovesUltimate[], waves: MagicWave[], sandTornadoes: SandTornado[], tombs: Tomb[], now: number, openedChests: number[], chestDrops: LootDrop[], attackProgress: number, loot: Point | null, droppedItem: Weapon | null, level: number, weapon: Weapon | null, weapon2: Weapon | null, armor: Weapon | null, facing: Point, moving: boolean, health: number, superReloading: boolean, profileName: string, secondHero: Point | null, secondFacing: Point, secondMoving: boolean, secondHealth: number, superReloading2: boolean, attackProgress2: number, armor2: Weapon | null, skin: HeroSkin, skin2: HeroSkin, explored: Point[], mobileControls = false, cameraFocus: Point = hero) {
  ...
  const cameraX = cameraFocus.x + 12 - viewWidth / 2;
  const cameraY = cameraFocus.y + 14 - viewHeight / 2; ctx.save(); ctx.scale(cameraZoom, cameraZoom); ctx.translate(-cameraX, -cameraY);
  ...
  if (!map.round) {
    ...
    ROUTE_POINTS.slice(1).forEach((end, index) => { const start = ROUTE_POINTS[index]; const dx = end.x - start.x, dy = end.y - start.y, length = Math.hypot(dx, dy), nx = -dy / length, ny = dx / length; for (let distance = 20; distance < length; distance += 52) { const px = start.x + dx * distance / length, py = start.y + dy * distance / length; borderPlant(ctx, px + nx * 88 - 30, py + ny * 88 - 30, desert ? '#4f8d43' : ice ? '#79ccef' : water ? '#405b32' : '#315f3c'); borderPlant(ctx, px - nx * 88 - 30, py - ny * 88 - 30, desert ? '#397a3c' : ice ? '#559fc8' : water ? '#344d2c' : '#284f35'); } });
  }
  ...
  map.walls.forEach((wall) => {
    ...
    ctx.fillStyle = desert ? '#624526' : ice ? '#315f79' : water ? '#263b24' : '#11271c'; ctx.fillRect(wall.x, wall.y, wall.w, wall.h);
    if (wall.w >= wall.h) for (let x = wall.x - 12; x < wall.x + wall.w; x += 48) wallPlant(ctx, x, wall.y + wall.h / 2 - 34, desert ? '#4b8b42' : ice ? '#69bde3' : water ? '#455f35' : map.floor[2]);
...
export function DungeonGame({ paused = false, enemyMultiplier = 1, startingCoins = 0, oneHitBoss = false, startingLevel: requestedStartingLevel, profileName, players = 1, playerClass = 'knight', playerClass2 = 'knight', initialSave, tutorial = false, merchantMode = false, mobileControls = false, equippedSkin = 'default', travelToLevel, saveRequest = 0, onSaveSnapshot, onVictory, onShopOpenChange, networkRole = null, remotePosition = null, onNetworkPosition }: { paused?: boolean; enemyMultiplier?: number; startingCoins?: number; oneHitBoss?: boolean; startingLevel?: number | null; profileName: string; players?: 1 | 2; playerClass?: PlayerClass; playerClass2?: PlayerClass; initialSave?: GameSave | null; tutorial?: boolean; merchantMode?: boolean; mobileControls?: boolean; equippedSkin?: string; travelToLevel?: number | null; saveRequest?: number; onSaveSnapshot?: (save: GameSave) => void; onVictory?: (level: number) => void; onShopOpenChange?: (open: boolean) => void; networkRole?: 'host' | 'guest' | null; remotePosition?: Point | null; onNetworkPosition?: (position: Point) => void }) {
  ...
  useEffect(() => { if (!remotePosition || !networkRole) return; const remote = networkRole === 'host' ? hero2.current : hero.current; remote.x = remotePosition.x; remote.y = remotePosition.y; }, [networkRole, remotePosition]);
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      if(networkRole&&onNetworkPosition&&now>=nextNetworkSendAt.current){nextNetworkSendAt.current=now+50;const local=networkRole==='guest'?p2:p;onNetworkPosition({x:local.x,y:local.y});}

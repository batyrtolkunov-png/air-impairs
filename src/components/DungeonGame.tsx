  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      sandTornadoes.current = sandTornadoes.current.filter((tornado) => {
        ...
        if(tornado.style==='bossSword'||tornado.style==='voidSword'){const source=tornado.source;if(!source||source.hp<=0)return false;if(now<(tornado.launchAt??0)){tornado.x=source.x;tornado.y=source.y-20;return true;}if(!tornado.startedAt){const firstDistance=health>0?Math.hypot(p.x+12-source.x,p.y+14-source.y):Infinity,secondDistance=players===2&&health2>0?Math.hypot(p2.x+12-source.x,p2.y+14-source.y):Infinity,target=secondDistance<firstDistance?p2:p,targetX=target.x+12-source.x,targetY=target.y+14-(source.y-20),targetDistance=Math.max(1,Math.hypot(targetX,targetY));tornado.x=source.x;tornado.y=source.y-20;tornado.vx=targetX/targetDistance*6.2;tornado.vy=targetY/targetDistance*6.2;tornado.startedAt=now;}if(now>=(tornado.impactAt??Infinity)){const returnX=source.x-tornado.x,returnY=source.y-20-tornado.y,returnDistance=Math.max(1,Math.hypot(returnX,returnY));if(returnDistance<24){if(tornado.style==='bossSword')source.thrownUntil=0;return false;}tornado.vx=returnX/returnDistance*6.2;tornado.vy=returnY/returnDistance*6.2;}}

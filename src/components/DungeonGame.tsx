...
function drawTinyPixelPetOverlay(ctx:CanvasRenderingContext2D,pet:PetKind,point:Point,camera:Point,attacking:boolean,now:number){const zoom=.5,viewW=640/zoom,viewH=360/zoom,cx=Math.round((camera.x+12-viewW/2)/2)*2,cy=Math.round((camera.y+14-viewH/2)/2)*2,pixelSize=14,jump=pet==='capybara'?Math.abs(Math.sin(now/170))*20:Math.abs(Math.sin(now/260))*5,lunge=attacking?18:0;const palettes={cat:['#17191d','#393c44','#72ef78'],crocodile:['#386335','#69a34c','#eff1bc'],pug:['#c9935e','#5a3b32','#48a14d'],capybara:['#a87348','#68452f','#f28b18']} as const;const shapes:Record<PetKind,string[]>={cat:['..0000..','.000000.','.002020.','00000000','00011000','.00..00.'],crocodile:['........','..111...','11111111','11000111','.022222.','11....11'],pug:['..2222..','..00000.','.000000.','.0011100','00011100','.00..00.'],capybara:['....22..','...2222.','..000000','00000001','00000111','.00..00.']};const palette=palettes[pet],shape=shapes[pet];ctx.save();ctx.imageSmoothingEnabled=false;ctx.scale(zoom,zoom);ctx.translate(-cx,-cy);ctx.translate(Math.round(point.x/pixelSize)*pixelSize+lunge,Math.round((point.y-jump)/pixelSize)*pixelSize);shape.forEach((row,y)=>[...row].forEach((cell,x)=>{if(cell==='.')return;ctx.fillStyle=palette[Number(cell) as 0|1|2];ctx.fillRect(x*pixelSize,y*pixelSize,pixelSize,pixelSize);}));ctx.fillStyle='#2b211c';if(pet==='cat')ctx.fillRect(0,0,pixelSize,pixelSize);if(pet==='crocodile')ctx.fillRect(8*pixelSize,3*pixelSize,pixelSize,pixelSize);if(attacking){ctx.fillStyle='#ffe16b';ctx.fillRect(8*pixelSize,2*pixelSize,pixelSize,pixelSize);ctx.fillRect(9*pixelSize,3*pixelSize,pixelSize,pixelSize);ctx.fillRect(8*pixelSize,4*pixelSize,pixelSize,pixelSize);}ctx.restore();}

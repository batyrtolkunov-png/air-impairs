...
function enemyHitRadius(enemy: Enemy) { return enemy.kind === 'boss' ? BOSS_HITBOX_RADIUS : enemy.kind === 'goblin' || enemy.kind === 'mummy' || enemy.kind === 'iceGolem' || enemy.kind === 'mudMonster' ? 18 : 14; }
...
function drawPalm(ctx: CanvasRenderingContext2D,x:number,y:number,tint:string){ctx.fillStyle='rgba(35,27,11,.3)';ctx.fillRect(x+5,y+55,57,8);ctx.fillStyle='#5b371f';ctx.fillRect(x+29,y+22,13,39);ctx.fillStyle='#a06b35';ctx.fillRect(x+33,y+23,6,37);ctx.fillStyle=tint;ctx.fillRect(x+27,y+8,14,22);ctx.fillRect(x+5,y+15,53,11);ctx.fillRect(x+14,y+5,17,14);ctx.fillRect(x+40,y+4,16,15);ctx.fillStyle='#2f713d';ctx.fillRect(x+8,y+18,22,6);ctx.fillRect(x+37,y+14,22,6);ctx.fillStyle='#d39b3d';ctx.fillRect(x+27,y+24,7,7);ctx.fillRect(x+38,y+23,7,7);}
...
function createEnemies(map: ReturnType<typeof getLevel>, multiplier = 1, oneHitBoss = false): Enemy[] {
  ...
  return spawns.map((enemy) => {
    const hp = enemy.kind === 'boss' ? oneHitBoss ? 1 : map.enemy.hp * (map.floor[0] === '#303944' ? 7.5 : 5) : enemy.kind === 'mudPile' ? 9999 : enemy.kind === 'mudMonster' ? 1 : enemy.kind === 'frog' ? 4 : enemy.kind === 'iceSpirit' ? 3 : enemy.kind === 'goblin' || enemy.kind === 'mummy' ? map.enemy.hp * 1.5 : enemy.kind === 'iceGolem' ? map.enemy.hp * .75 : map.enemy.hp;
    const speed = enemy.kind === 'boss' ? map.enemy.speed * .65 : enemy.kind === 'mudPile' ? 0 : enemy.kind === 'mudMonster' ? Math.max(1.05, map.enemy.speed) : enemy.kind === 'goblin' || enemy.kind === 'iceGolem' ? map.enemy.speed * 1.25 : enemy.kind === 'mummy' ? map.enemy.speed * .8 : map.enemy.speed;
    return { ...enemy, ...map.enemy, color: enemy.kind === 'slime' ? map.enemy.color : enemy.kind === 'frog' ? '#58a94f' : enemy.kind === 'mudPile' || enemy.kind === 'mudMonster' ? '#60452f' : enemy.kind === 'iceSpirit' ? '#71d5f2' : enemy.kind === 'mummy' ? '#d8c79a' : enemy.kind === 'scorpion' ? '#a95d2e' : enemy.kind === 'iceGolem' ? '#6bc8e8' : '#69ad68', hp, maxHp: hp, power: enemy.kind === 'mudPile' || enemy.kind === 'mudMonster' ? 5 : enemy.kind === 'boss' ? 2 : enemy.kind === 'iceSpirit' ? 1 : enemy.kind === 'mummy' ? Math.max(.5, map.enemy.power / 2) : enemy.kind === 'iceGolem' ? 1.3 : map.enemy.power, speed, flash: 0, attackUntil: 0, stunnedUntil: 0, leapStarted: 0, leapUntil: 0, leapTargetX: 0, leapTargetY: 0, nextLeapAt: 0, nextShotAt: 0, nextSummonAt: 0, nextContactAt: 0, playerWasInSummonRadius: false, revived: false, reviveFlashUntil: 0 };
  });
}
...
function drawScene(ctx: CanvasRenderingContext2D, map: ReturnType<typeof getLevel>, hero: Point, enemies: Enemy[], projectiles: Projectile[], superFists: SuperFist[], swordUltimates: SwordUltimate[], bowUltimates: BowUltimate[], staffUltimates: StaffUltimate[], glovesUltimates: GlovesUltimate[], waves: MagicWave[], sandTornadoes: SandTornado[], tombs: Tomb[], now: number, openedChests: number[], chestDrops: LootDrop[], attackProgress: number, loot: Point | null, droppedItem: Weapon | null, level: number, weapon: Weapon | null, weapon2: Weapon | null, armor: Weapon | null, facing: Point, moving: boolean, health: number, superReloading: boolean, profileName: string, secondHero: Point | null, secondFacing: Point, secondMoving: boolean, secondHealth: number, superReloading2: boolean, attackProgress2: number, armor2: Weapon | null, skin: HeroSkin, skin2: HeroSkin, explored: Point[], mobileControls = false, cameraFocus: Point = hero) {
  ...
  if (!map.round) {
    const iceRegion = level >= 13 && level <= 18, coastalRegion=level===19||level===20, mudRegion = level >= 19 && level <= 24&&!coastalRegion; ctx.fillStyle = coastalRegion?'#176f9b':iceRegion ? 'rgba(35,94,119,.38)' : 'rgba(4,10,7,.78)'; ctx.fillRect(32, 32, map.worldWidth - 64, map.worldHeight - 64); ctx.strokeStyle = coastalRegion?'#0d557f':iceRegion ? '#477f9a' : mudRegion ? '#3d2a20' : '#17271d'; ctx.lineWidth = coastalRegion?184:158; ctx.lineJoin = 'round'; ctx.lineCap = 'round'; ctx.beginPath(); ROUTE_POINTS.forEach((point, index) => { if (index === 0) ctx.moveTo(point.x, point.y); else ctx.lineTo(point.x, point.y); }); ctx.stroke();if(coastalRegion){ctx.strokeStyle='#4fc3dc';ctx.lineWidth=158;ctx.stroke();ctx.setLineDash([22,18]);ctx.lineDashOffset=-(now/45)%40;ctx.strokeStyle='#d9fbff';ctx.lineWidth=5;ctx.stroke();ctx.setLineDash([]);} ctx.strokeStyle = coastalRegion?'#e7c36c':mudRegion ? '#765037' : map.floor[1]; ctx.lineWidth = 136; ctx.stroke();
    ...
    if(!coastalRegion)ROUTE_POINTS.slice(1).forEach((end, index) => { const start = ROUTE_POINTS[index]; const dx = end.x - start.x, dy = end.y - start.y, length = Math.hypot(dx, dy), nx = -dy / length, ny = dx / length; for (let distance = 20; distance < length; distance += 52) { const px = start.x + dx * distance / length, py = start.y + dy * distance / length; borderPlant(ctx, px + nx * 88 - 30, py + ny * 88 - 30, desert ? '#4f8d43' : ice ? '#79ccef' : water ? '#405b32' : '#315f3c'); borderPlant(ctx, px - nx * 88 - 30, py - ny * 88 - 30, desert ? '#397a3c' : ice ? '#559fc8' : water ? '#344d2c' : '#284f35'); } });
  }
  ...
  map.walls.forEach((wall,wallIndex) => {
    if((level===19||level===20)&&wallIndex<4)return;
    const desert = level >= 7 && level <= 12, ice = level >= 13 && level <= 18, coast=level===19||level===20, water = level >= 19 && level <= 24; const wallPlant = desert ? drawCactus : ice ? drawIceSpire : coast?drawPalm:water ? drawWaterBarrier : drawTree;
  ...
  enemies.forEach((e) => {
    ...
    else { ctx.translate(e.x + 1 + (hero.x - e.x) / distance * force, e.y - 5 + (hero.y - e.y) / distance * force); ctx.scale(e.kind === 'goblin' || e.kind === 'mummy' || e.kind === 'iceGolem' || e.kind === 'iceSpirit' || e.kind === 'frog' || e.kind === 'mudPile' || e.kind === 'mudMonster' ? .38 : e.kind === 'scorpion' ? .28 : .21, e.kind === 'goblin' || e.kind === 'mummy' || e.kind === 'iceGolem' || e.kind === 'iceSpirit' || e.kind === 'frog' || e.kind === 'mudPile' || e.kind === 'mudMonster' ? .38 : e.kind === 'scorpion' ? .28 : .21); }
...
export function DungeonGame({ paused = false, enemyMultiplier = 1, startingCoins = 0, oneHitBoss = false, startingLevel: requestedStartingLevel, profileName, players = 1, playerClass = 'knight', playerClass2 = 'knight', initialSave, tutorial = false, merchantMode = false, mobileControls = false, equippedSkin = 'default', travelToLevel, saveRequest = 0, onSaveSnapshot, onVictory, onShopOpenChange, networkRole = null, remotePosition = null, onNetworkPosition, remoteGameState = null, onNetworkGameState, keyBindings = DEFAULT_KEY_BINDINGS }: { paused?: boolean; enemyMultiplier?: number; startingCoins?: number; oneHitBoss?: boolean; startingLevel?: number | null; profileName: string; players?: 1 | 2; playerClass?: PlayerClass; playerClass2?: PlayerClass; initialSave?: GameSave | null; tutorial?: boolean; merchantMode?: boolean; mobileControls?: boolean; equippedSkin?: string; travelToLevel?: number | null; saveRequest?: number; onSaveSnapshot?: (save: GameSave) => void; onVictory?: (level: number) => void; onShopOpenChange?: (open: boolean) => void; networkRole?: 'host' | 'guest' | null; remotePosition?: (Point & { fx?: number; fy?: number; moving?: boolean }) | null; onNetworkPosition?: (position: Point & { fx?: number; fy?: number; moving?: boolean }) => void; remoteGameState?: NetworkGameState | null; onNetworkGameState?: (state: NetworkGameState) => void; keyBindings?: KeyBindings }) {
  ...
  useEffect(() => {
    ...
    const loop = (now: number) => {
      ...
      drawScene(ctx, map, p, enemies.current, projectiles.current, superFists.current, swordUltimates.current, bowUltimates.current, staffUltimates.current, glovesUltimates.current, waves.current, sandTornadoes.current, tombs.current, now, openedChests, chestDrops, attackProgress, loot, droppedItem, level, weapon, weapon2, armor, facing.current, Boolean(dx || dy) || networkRole==='guest'&&now<remoteMovingUntil.current, health, superReloading, profileName, players === 2 ? p2 : null, facing2.current, Boolean(dx2 || dy2) || networkRole==='host'&&now<remoteMovingUntil.current, health2, superReloading2, attackProgress2, armor2, skin, skin2, explored.current, mobileControls, networkRole==='guest'?p2:p); frame = requestAnimationFrame(loop);
    };
    frame = requestAnimationFrame(loop); return () => cancelAnimationFrame(frame);
  }, [armor, armor2, chestDrops, choiceItem, dead, droppedItem, enemyMultiplier, health, health2, level, loot, merchantMode, mobileControls, networkRole, openedChests, paused, players, profileName, shopOpen, showInventory, skin, skin2, superReloading, superReloading2, victory, weapon, weapon2]);

    ...
    const loop = (now: number) => {
      ...
      enemies.current.forEach((e) => {
        ...
        if(level===30&&e.kind==='boss'){
          ...
          if(distance<=160&&now>=(e.nextShotAt??0)){e.nextShotAt=now+2800;e.attackUntil=now+1000;e.bossAttack='spin';const hitFirst=health>0&&Math.hypot(p.x+12-e.x,p.y+14-e.y)<=160,hitSecond=players===2&&health2>0&&Math.hypot(p2.x+12-e.x,p2.y+14-e.y)<=160;if(hitFirst||hitSecond)damageHero(7,'Хранитель замка раскрутил длинный топор и нанёс 7 HP урона!',!hitFirst&&hitSecond);setMessage('Хранитель замка вращает длинный топор — отойди дальше пяти клеток!');}

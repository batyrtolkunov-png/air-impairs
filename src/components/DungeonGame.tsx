...
function drawScene(ctx: CanvasRenderingContext2D, map: ReturnType<typeof getLevel>, hero: Point, enemies: Enemy[], projectiles: Projectile[], superFists: SuperFist[], swordUltimates: SwordUltimate[], bowUltimates: BowUltimate[], staffUltimates: StaffUltimate[], glovesUltimates: GlovesUltimate[], waves: MagicWave[], sandTornadoes: SandTornado[], tombs: Tomb[], now: number, openedChests: number[], chestDrops: LootDrop[], attackProgress: number, loot: Point | null, droppedItem: Weapon | null, level: number, weapon: Weapon | null, weapon2: Weapon | null, armor: Weapon | null, facing: Point, moving: boolean, health: number, superReloading: boolean, profileName: string, secondHero: Point | null, secondFacing: Point, secondMoving: boolean, secondHealth: number, superReloading2: boolean, attackProgress2: number, armor2: Weapon | null, skin: HeroSkin, skin2: HeroSkin, explored: Point[], mobileControls = false, cameraFocus: Point = hero) {
  ...
  if (!map.round) {
    const iceRegion = level >= 13 && level <= 18, coastalRegion=level===25||level===26, mudRegion = level >= 19 && level <= 24; ctx.fillStyle = coastalRegion?'#176f9b':iceRegion ? 'rgba(35,94,119,.38)' : 'rgba(4,10,7,.78)'; ctx.fillRect(32, 32, map.worldWidth - 64, map.worldHeight - 64); ctx.strokeStyle = coastalRegion?'#0d557f':iceRegion ? '#477f9a' : mudRegion ? '#3d2a20' : '#17271d'; ctx.lineWidth = coastalRegion?260:158; ctx.lineJoin = 'round'; ctx.lineCap = 'round'; ctx.beginPath(); ROUTE_POINTS.forEach((point, index) => { if (index === 0) ctx.moveTo(point.x, point.y); else ctx.lineTo(point.x, point.y); }); ctx.stroke();if(coastalRegion){ctx.strokeStyle='#4fc3dc';ctx.lineWidth=210;ctx.stroke();ctx.setLineDash([26,20]);ctx.lineDashOffset=-(now/45)%46;ctx.strokeStyle='#d9fbff';ctx.lineWidth=8;ctx.stroke();ctx.setLineDash([]);} ctx.strokeStyle = coastalRegion?'#e7c36c':mudRegion ? '#765037' : map.floor[1]; ctx.lineWidth = 136; ctx.stroke();
  ...
  map.walls.forEach((wall,wallIndex) => {
    ...
    const desert = level >= 7 && level <= 12, ice = level >= 13 && level <= 18, coast=level===25||level===26, water = level >= 19 && level <= 24; const wallPlant = desert ? drawCactus : ice ? drawIceSpire : coast?drawPalm:water ? drawWaterBarrier : drawTree;

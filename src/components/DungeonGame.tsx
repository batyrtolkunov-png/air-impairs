...
function drawPetOverlay(ctx:CanvasRenderingContext2D,pet:PetKind,point:Point,camera:Point,attacking:boolean,now:number){const zoom=.5,viewW=640/zoom,viewH=360/zoom,cx=Math.round((camera.x+12-viewW/2)/2)*2,cy=Math.round((camera.y+14-viewH/2)/2)*2;ctx.save();ctx.scale(zoom,zoom);ctx.translate(-cx,-cy);ctx.translate(point.x,point.y-(pet==='capybara'?Math.abs(Math.sin(now/160))*13:0));if(attacking)ctx.translate(11,0);ctx.strokeStyle='#30231d';ctx.lineWidth=3;if(pet==='cat'){ctx.fillStyle='#17191d';ctx.fillRect(5,9,24,17);ctx.fillRect(18,1,17,17);ctx.beginPath();ctx.moveTo(18,3);ctx.lineTo(21,-7);ctx.lineTo(26,3);ctx.moveTo(29,3);ctx.lineTo(34,-7);ctx.lineTo(37,5);ctx.fill();ctx.fillStyle='#72ef78';ctx.fillRect(23,7,3,3);ctx.fillRect(31,7,3,3);}else if(pet==='crocodile'){ctx.fillStyle='#47753b';ctx.fillRect(1,12,35,18);ctx.fillRect(27,8,24,15);ctx.fillStyle='#d6e76b';ctx.fillRect(35,11,4,4);ctx.fillStyle='#fff1ce';for(let x=30;x<49;x+=7)ctx.fillRect(x,23,4,4);}else if(pet==='pug'){ctx.fillStyle='#c99561';ctx.fillRect(4,13,31,19);ctx.fillStyle='#dcb17b';ctx.fillRect(24,4,23,25);ctx.fillStyle='#55382f';ctx.fillRect(36,15,16,11);ctx.fillStyle='#48a14d';ctx.fillRect(22,-11,29,16);ctx.fillStyle='#226e35';ctx.fillRect(29,-9,5,13);ctx.fillRect(40,-9,5,13);}else{ctx.fillStyle='#a87348';ctx.fillRect(1,12,38,21);ctx.fillRect(30,5,25,25);ctx.fillStyle='#875a3d';ctx.fillRect(48,17,13,10);ctx.fillStyle='#f28b18';ctx.beginPath();ctx.arc(42,-2,11,0,Math.PI*2);ctx.fill();ctx.fillStyle='#4f983f';ctx.fillRect(41,-16,4,7);}ctx.restore();}

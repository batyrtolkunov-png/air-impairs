...
function drawSandTornado(ctx: CanvasRenderingContext2D, tornado: SandTornado, now: number) {
  ...
  if(tornado.style==='fallingSword'){ctx.save();ctx.translate(tornado.x,tornado.y);if(now<(tornado.impactAt??0)){ctx.fillStyle='rgba(150,44,64,.2)';ctx.beginPath();ctx.arc(0,0,18,0,Math.PI*2);ctx.fill();ctx.strokeStyle='#ff5268';ctx.lineWidth=3;ctx.stroke();ctx.fillStyle='#fff';ctx.fillRect(-1,-11,2,22);ctx.fillRect(-11,-1,22,2);}else{ctx.fillStyle='#ccd8de';ctx.beginPath();ctx.moveTo(0,-32);ctx.lineTo(7,7);ctx.lineTo(0,17);ctx.lineTo(-7,7);ctx.closePath();ctx.fill();ctx.fillStyle='#fff';ctx.fillRect(-2,-25,4,27);ctx.fillStyle='#d4b354';ctx.fillRect(-13,8,26,5);}ctx.restore();return;}
    ...
    const loop = (now: number) => {
      ...
      enemies.current.forEach((e) => {
        ...
        if(level===30&&e.kind==='boss'){
          if(e.bossAttack==='sword'){if(now>=(e.nextShotAt??0)){const half=e.hp<=e.maxHp/2,length=Math.max(1,distance),count=half?2:1;e.nextShotAt=now+7000;e.attackUntil=now+650;for(let swordIndex=0;swordIndex<count;swordIndex++){const launchAt=now+swordIndex*1000;sandTornadoes.current.push({x:e.x,y:e.y-20,vx:ex/length*6.2,vy:ey/length*6.2,damage:7,launchAt,impactAt:launchAt+750,until:launchAt+3000,style:swordIndex===1?'voidSword':'bossSword',source:e});}setMessage(half?'Страж бросает два возвращающихся меча с интервалом в одну секунду!':'Страж метнул меч в игрока — он вернётся обратно!');}if(e.hp<=e.maxHp/2&&now>=(e.nextSummonAt??0)){e.nextSummonAt=now+5000;for(let swordIndex=0;swordIndex<10;swordIndex++){const angle=Math.random()*Math.PI*2,radius=Math.sqrt(Math.random())*245;sandTornadoes.current.push({x:320+Math.cos(angle)*radius,y:336+Math.sin(angle)*radius,vx:0,vy:0,damage:2,impactAt:now+500,until:now+1050,style:'fallingSword'});}setMessage('Через полсекунды с потолка упадут десять мечей!');}if(e.attackUntil<=now&&distance>46){const step=Math.max(.75,e.speed)*dt,moveX=e.x+ex/distance*step,moveY=e.y+ey/distance*step;if(Math.hypot(moveX-320,moveY-336)<275){e.x=moveX;e.y=moveY;}}return;}
      ...
      sandTornadoes.current = sandTornadoes.current.filter((tornado) => {
        ...
        if(tornado.style==='bossSword'||tornado.style==='voidSword'){const source=tornado.source;if(!source||source.hp<=0)return false;if(now<(tornado.launchAt??0)){tornado.x=source.x;tornado.y=source.y-20;return true;}if(now>=(tornado.impactAt??Infinity)){const returnX=source.x-tornado.x,returnY=source.y-20-tornado.y,returnDistance=Math.max(1,Math.hypot(returnX,returnY));if(returnDistance<24)return false;tornado.vx=returnX/returnDistance*6.2;tornado.vy=returnY/returnDistance*6.2;}}

    ...
    const loop = (now: number) => {
      ...
      enemies.current.forEach((e) => {
        ...
        if(level===30&&e.kind==='boss'){
          if(e.bossAttack==='sword'){if(distance<=70&&e.attackUntil<=now&&now>=(e.nextShotAt??0)){e.nextShotAt=now+7000;e.attackUntil=now+650;const hitFirst=health>0&&Math.hypot(p.x+12-e.x,p.y+14-e.y)<=70,hitSecond=players===2&&health2>0&&Math.hypot(p2.x+12-e.x,p2.y+14-e.y)<=70;if(hitFirst||hitSecond)damageHero(7,'Второй хранитель рубанул мечом и нанёс 7 HP урона!',!hitFirst&&hitSecond);setMessage('Страж с мечом атаковал — следующая атака через семь секунд!');}if(e.attackUntil<=now&&distance>46){const step=Math.max(.75,e.speed)*dt,moveX=e.x+ex/distance*step,moveY=e.y+ey/distance*step;if(Math.hypot(moveX-320,moveY-336)<275){e.x=moveX;e.y=moveY;}}return;}
      ...
      sandTornadoes.current = sandTornadoes.current.filter((tornado) => {
        ...
        const radius = tornado.style === 'voidSaw' ? 32 : tornado.style === 'iceLarge' ? 25 : tornado.style === 'iceSpiritDrop' ? 16 : tornado.style === 'iceRing' ? 12 : tornado.style === 'iceShard' ? 13 : tornado.style === 'frogTongue' ? 8 : 22;
        const canSawHit=tornado.style!=='voidSaw'||now>=(tornado.hitCooldownUntil??0);const hitFirst = canSawHit&&!waitingToFall && health > 0 && !tornado.hitPlayer1 && Math.hypot(p.x + 12 - tornado.x, p.y + 14 - tornado.y) < radius;
        ...
        if (hitFirst || hitSecond) {
          ...
          damageHero(tornado.damage, tornado.style === 'voidSaw' ? 'Чёрно-фиолетовая пила нанесла 4 HP урона!' : tornado.style === 'frogTongue' ? `Язык жабы нанёс ${tornado.damage} HP урона!` : iceShot ? `Сосулька нанесла ${tornado.damage} HP урона!` : 'Песчаный торнадо настиг героя!', !hitFirst && hitSecond);
          if(tornado.style==='voidSaw'){tornado.hitCooldownUntil=now+900;return true;}

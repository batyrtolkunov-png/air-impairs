...
function drawScene(ctx: CanvasRenderingContext2D, map: ReturnType<typeof getLevel>, hero: Point, enemies: Enemy[], projectiles: Projectile[], superFists: SuperFist[], swordUltimates: SwordUltimate[], bowUltimates: BowUltimate[], staffUltimates: StaffUltimate[], glovesUltimates: GlovesUltimate[], waves: MagicWave[], sandTornadoes: SandTornado[], tombs: Tomb[], now: number, openedChests: number[], chestDrops: LootDrop[], attackProgress: number, loot: Point | null, droppedItem: Weapon | null, level: number, weapon: Weapon | null, weapon2: Weapon | null, armor: Weapon | null, facing: Point, moving: boolean, health: number, superReloading: boolean, profileName: string, secondHero: Point | null, secondFacing: Point, secondMoving: boolean, secondHealth: number, superReloading2: boolean, attackProgress2: number, armor2: Weapon | null, skin: HeroSkin, skin2: HeroSkin, explored: Point[], mobileControls = false, cameraFocus: Point = hero, castleGuardVisible = true) {
  ...
  if(finalEncounter&&finalElapsed>=2800){ctx.save();ctx.beginPath();ctx.arc(320,336,306,0,Math.PI*2);ctx.clip();ctx.fillStyle='#3a090d';ctx.fillRect(0,0,map.worldWidth,map.worldHeight);ctx.fillStyle='#671016';for(let y=20;y<map.worldHeight;y+=54)for(let x=18;x<map.worldWidth;x+=68){ctx.fillRect(x+(y%3)*7,y,38,24);ctx.fillStyle='#a82418';ctx.fillRect(x+8,y+22,28,4);ctx.fillStyle='#671016';}ctx.strokeStyle='#ff5128';ctx.lineWidth=5;for(let i=0;i<13;i++){ctx.beginPath();ctx.moveTo((i*97)%map.worldWidth,0);ctx.lineTo((i*71+80)%map.worldWidth,map.worldHeight);ctx.stroke();}ctx.fillStyle='rgba(255,72,25,.15)';for(let i=0;i<18;i++){ctx.beginPath();ctx.arc((i*83)%map.worldWidth,(i*127)%map.worldHeight,18+(i%3)*8,0,Math.PI*2);ctx.fill();}ctx.restore();ctx.save();ctx.shadowColor='#ff2d20';ctx.shadowBlur=22;ctx.strokeStyle='#5a1117';ctx.lineWidth=28;ctx.beginPath();ctx.arc(320,336,306,0,Math.PI*2);ctx.stroke();ctx.strokeStyle='#ff4b27';ctx.lineWidth=7;ctx.stroke();ctx.strokeStyle='#ffb23f';ctx.lineWidth=2;ctx.setLineDash([12,10]);ctx.stroke();ctx.restore();}
  enemies.forEach((e) => {
  ...
  if(finalEncounter?.finalAttack===2){const progress=Math.max(0,Math.min(1,(now-(finalEncounter.finalAttackStarted??now))/1200)),angle=Math.atan2(finalEncounter.thrownTargetY??0,finalEncounter.thrownTargetX??1)+(progress-.5)*1.1;ctx.save();ctx.translate(finalEncounter.x,finalEncounter.y);ctx.rotate(angle);ctx.fillStyle='rgba(255,45,68,.34)';ctx.beginPath();ctx.arc(82,0,55,0,Math.PI*2);ctx.fill();ctx.fillStyle='#271020';ctx.fillRect(42,-21,65,42);for(let i=-2;i<=2;i++)ctx.fillRect(86,i*13-5,44,9);ctx.strokeStyle='#f04b64';ctx.lineWidth=4;ctx.strokeRect(42,-21,65,42);ctx.restore();}
        ...
        if(level===30&&e.kind==='boss'){
          if((e.bossAttack as string)==='finalBoss'){
            ...
            if(!e.finalAttack&&now>=(e.nextShotAt??0)){
              ...
              if(attack===2){e.attackUntil=now+1200;const d=Math.max(1,distance);e.thrownTargetX=ex/d;e.thrownTargetY=ey/d;setMessage('Уничтожитель занёс раскрытую ладонь для широкого удара!');}
            ...
            if(e.finalAttack===2&&now-(e.finalAttackStarted??now)>500&&now>=(e.nextContactAt??0)){const hit=(h:Point)=>{const hx=h.x+12-e.x,hy=h.y+14-e.y;return Math.hypot(hx,hy)<=128&&hx*(e.thrownTargetX??1)+hy*(e.thrownTargetY??0)>0;};if(hit(p)||(players===2&&hit(p2))){damageHero(6,'Удар раскрытой ладонью нанёс 6 HP!',!hit(p)&&players===2&&hit(p2));e.nextContactAt=e.attackUntil+1;}}
            return;
          }

...
export type EnemyKind = 'slime' | 'goblin' | 'mummy' | 'scorpion' | 'iceGolem' | 'iceSpirit' | 'frog' | 'boss';
...
export function getLevel(number: number): LevelConfig {
  ...
  const enemies: EnemySpawn[] = round
    ...
    : Array.from({ length: Math.floor((1 + localNumber) * 20 * (regionIndex === 2 ? .4 : 1)) }, (_, index) => ({
      ...safePoint(), kind: index % 4 === 3 ? (regionIndex === 1 ? 'mummy' as const : regionIndex === 2 ? 'iceGolem' as const : 'goblin' as const) : (regionIndex === 1 ? 'scorpion' as const : regionIndex === 2 ? 'iceSpirit' as const : regionIndex === 3 ? 'frog' as const : 'slime' as const),
    }));

...
export async function talkToCastleGuard(history: GuardLine[], message: string): Promise<GuardReply> {
  const transcript = [...history, { speaker: 'player' as const, text: message }]
    .slice(-10).map((line) => `${line.speaker === 'player' ? 'Игрок' : 'Рыцари'}: ${line.text}`).join('\n');
  const system = `Ты говоришь от лица двух суровых и грубоватых рыцарей-стражей у ворот замка в пиксельной фэнтези-игре. Отвечай по-русски, кратко, 1-2 предложениями, иногда показывая разногласие между рыцарями, без ненависти к реальным группам и без нецензурной брани. Оцени весь разговор.
...
fight — игрок оскорбляет, угрожает, вызывает на бой или упорно провоцирует; peace — игрок убедил пропустить его мирно; farewell — стороны решили разойтись и попрощались без прохода; continue — решения пока нет.`;
  ...
  if (typeof parsed.reply !== 'string' || !outcomes.has(parsed.outcome as GuardOutcome)) throw new Error('Некорректный ответ стражей');
